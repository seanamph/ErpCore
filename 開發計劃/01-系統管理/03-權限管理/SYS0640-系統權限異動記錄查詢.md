...
- **功能代碼**: SYS0640
- **功能名稱**: 系統權限異動記錄查詢
- **功能描述**: 提供系統權限異動記錄的查詢功能，可查詢指定使用者或角色的系統權限異動歷史記錄
- **參考舊程式**: 
  - `WEB/IMS_CORE/ASP/SYS0000/SYS0640_FQ.asp` (查詢條件頁面)
  - `WEB/IMS_CORE/ASP/SYS0000/SYS0640_FB.ASP` (查詢結果頁面)
  - `WEB/IMS_CORE/SYS0000/SYS0640_FB.aspx` (ASP.NET 版本)
...
CREATE TABLE [dbo].[ChangeLogs] (
    ...
    [ProgramId] NVARCHAR(50) NOT NULL, -- 程式代碼 (SYS0310, SYS0320)
...
#### 2.2.3 `SystemPermissions` - 系統權限表
- 用於查詢系統權限資訊
- 參考: `開發計劃/01-系統管理/03-權限管理/SYS0310-角色系統權限設定.md`
...
| ProgramId | NVARCHAR | 50 | NO | - | 程式代碼 | SYS0310, SYS0320 |
...
#### 3.1.1 查詢系統權限異動記錄
- **HTTP 方法**: `GET`
- **路徑**: `/api/v1/change-logs/system-permissions`
- **說明**: 查詢系統權限異動記錄列表，支援分頁、排序、篩選
- **請求參數**:
  ```json
  {
    "pageIndex": 1,
...
#### 3.1.2 查詢單筆系統權限異動記錄
- **HTTP 方法**: `GET`
- **路徑**: `/api/v1/change-logs/system-permissions/{logId}`
- **說明**: 根據主鍵查詢單筆系統權限異動記錄
- **回應格式**: 標準單筆回應格式

#### 3.1.3 匯出系統權限異動記錄報表
- **HTTP 方法**: `POST`
- **路徑**: `/api/v1/change-logs/system-permissions/export`
- **說明**: 匯出系統權限異動記錄報表 (Excel/PDF)
- **請求格式**: 同查詢參數
- **回應格式**: 檔案下載
...
#### 4.1.1 系統權限異動記錄查詢頁面 (`SystemPermissionChangeLogQuery.vue`)
- **路徑**: `/system/change-logs/system-permissions`
- **功能**: 顯示系統權限異動記錄查詢表單與結果列表
- **主要元件**:
  - 查詢表單 (SystemPermissionChangeLogSearchForm)
  - 資料表格 (SystemPermissionChangeLogDataTable)
  - 匯出按鈕
...
### 6.3 業務邏輯
- 查詢條件需包含 SYS0310 和 SYS0320 兩個程式代碼
- 異動前的值和異動後的值需解析並顯示系統權限資訊
...
### 8.1 舊程式碼
- `WEB/IMS_CORE/ASP/SYS0000/SYS0640_FQ.asp`
- `WEB/IMS_CORE/ASP/SYS0000/SYS0640_FB.ASP`
- `WEB/IMS_CORE/SYS0000/SYS0640_FB.aspx`
