...
- **功能代碼**: SYS0660
- **功能名稱**: 其他異動記錄查詢
- **功能描述**: 提供系統功能按鍵等其他異動記錄的查詢功能，可查詢指定使用者或作業的異動歷史記錄
- **參考舊程式**: 
  - `WEB/IMS_CORE/ASP/SYS0000/SYS0660_FQ.asp` (查詢條件頁面)
  - `WEB/IMS_CORE/ASP/SYS0000/SYS0660_FB.ASP` (查詢結果頁面)
...
CREATE TABLE [dbo].[ChangeLogs] (
    ...
    [ProgramId] NVARCHAR(50) NOT NULL, -- 程式代碼 (多種程式)
...
| ProgramId | NVARCHAR | 50 | NO | - | 程式代碼 | 多種程式 |
...
#### 3.1.1 查詢其他異動記錄
- **HTTP 方法**: `GET`
- **路徑**: `/api/v1/change-logs/others`
- **說明**: 查詢其他異動記錄列表，支援分頁、排序、篩選
- **請求參數**:
  ```json
  {
    "pageIndex": 1,
...
#### 3.1.2 查詢單筆其他異動記錄
- **HTTP 方法**: `GET`
- **路徑**: `/api/v1/change-logs/others/{logId}`
- **說明**: 根據主鍵查詢單筆其他異動記錄
- **回應格式**: 標準單筆回應格式

#### 3.1.3 匯出其他異動記錄報表
- **HTTP 方法**: `POST`
- **路徑**: `/api/v1/change-logs/others/export`
- **說明**: 匯出其他異動記錄報表 (Excel/PDF)
- **請求格式**: 同查詢參數
- **回應格式**: 檔案下載
...
#### 4.1.1 其他異動記錄查詢頁面 (`OtherChangeLogQuery.vue`)
- **路徑**: `/system/change-logs/others`
- **功能**: 顯示其他異動記錄查詢表單與結果列表
- **主要元件**:
  - 查詢表單 (OtherChangeLogSearchForm)
  - 資料表格 (OtherChangeLogDataTable)
  - 匯出按鈕
...
#### 4.2.2 資料表格元件
- 顯示異動時間、異動者、作業代碼、作業名稱、異動狀態、異動欄位、異動前的值、異動後的值
- 支援排序、分頁
...
### 6.3 業務邏輯
- 查詢條件可包含多種程式代碼（排除已專門處理的程式）
- 異動前的值和異動後的值需解析並顯示作業資訊
- 支援依作業代碼模糊查詢
- 日期範圍查詢需包含結束日期當天
...
### 8.1 舊程式碼
- `WEB/IMS_CORE/ASP/SYS0000/SYS0660_FQ.asp`
- `WEB/IMS_CORE/ASP/SYS0000/SYS0660_FB.ASP`
