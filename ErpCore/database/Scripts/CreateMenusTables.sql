...
BEGIN
    ...
    BEGIN
        ...
        ADD CONSTRAINT [FK_Menus_Systems] FOREIGN KEY ([SystemId]) REFERENCES [dbo].[Systems] ([SystemId]);
    END

    -- 自引用外鍵約束（上層子系統）
    ALTER TABLE [dbo].[Menus]
    ADD CONSTRAINT [FK_Menus_ParentMenu] FOREIGN KEY ([ParentMenuId]) REFERENCES [dbo].[Menus] ([MenuId]);
