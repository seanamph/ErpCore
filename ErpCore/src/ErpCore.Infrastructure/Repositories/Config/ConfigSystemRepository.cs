...
namespace ErpCore.Infrastructure.Repositories.Config;

/// <summary>
/// 主系統項目 Repository 實作 (CFG0410)
/// 使用 Dapper 進行資料庫存取
/// </summary>
public class ConfigSystemRepository : BaseRepository, IConfigSystemRepository
{
    ...
    public async Task<PagedResult<ConfigSystem>> QueryAsync(ConfigSystemQuery query)
    {
        try
        {
            ...
            // 排序
            var sortField = string.IsNullOrEmpty(query.SortField) ? "SystemId" : query.SortField;
