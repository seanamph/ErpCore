...
/// <summary>
/// STD3000 資料 Repository 實作 (SYS3620 - 標準資料維護)
/// 使用 Dapper 進行資料庫存取
/// </summary>
public class Std3000DataRepository : BaseRepository, IStd3000DataRepository
{
    public Std3000DataRepository(IDbConnectionFactory connectionFactory, ILoggerService logger)
        : base(connectionFactory, logger)
    {
    }

    public async Task<Std3000Data?> GetByIdAsync(long tKey)
    {
        try
        {
            const string sql = @"
                SELECT * FROM Std3000Data 
                WHERE TKey = @TKey";

            return await QueryFirstOrDefaultAsync<Std3000Data>(sql, new { TKey = tKey });
        }
        catch (Exception ex)
        {
    ...
    public async Task<Std3000Data?> GetByDataIdAsync(string dataId)
    {
        try
        {
            const string sql = @"
            ...
            return await QueryFirstOrDefaultAsync<Std3000Data>(sql, new { DataId = dataId });
        }
        catch (Exception ex)
        {
    ...
    public async Task<IEnumerable<Std3000Data>> QueryAsync(Std3000DataQuery query)
    {
        try
        {
            var sql = @"
            ...
            {
                sql += " ORDER BY SortOrder ASC, DataId ASC";
            }
            ...
            return await QueryAsync<Std3000Data>(sql, parameters);
        }
        catch (Exception ex)
        {
    ...
    public async Task<int> GetCountAsync(Std3000DataQuery query)
    {
        try
        {
            var sql = @"
                SELECT COUNT(*) FROM Std3000Data
                WHERE 1=1";
        ...
        {
            _logger.LogError("查詢STD3000資料總數失敗", ex);
            throw;
        }
    }

    public async Task<long> CreateAsync(Std3000Data entity)
    {
        try
        {
            const string sql = @"
                INSERT INTO Std3000Data 
    ...
    public async Task UpdateAsync(Std3000Data entity)
    {
        try
        {
            const string sql = @"
        ...
        {
            _logger.LogError($"修改STD3000資料失敗: {entity.TKey}", ex);
            throw;
        }
    }
