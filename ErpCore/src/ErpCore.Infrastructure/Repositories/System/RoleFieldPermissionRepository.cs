...
/// <summary>
/// 角色欄位權限 Repository 實作 (SYS0330)
/// </summary>
public class RoleFieldPermissionRepository : BaseRepository, IRoleFieldPermissionRepository
{
    public RoleFieldPermissionRepository(IDbConnectionFactory connectionFactory, ILoggerService logger)
        : base(connectionFactory, logger)
    {
    }

    public async Task<PagedResult<RoleFieldPermission>> QueryAsync(RoleFieldPermissionQuery query)
    {
        try
        {
            var sql = @"
            ...
            var items = await QueryAsync<RoleFieldPermission>(sql, parameters);

            // 查詢總數
            var countSql = "SELECT COUNT(*) FROM RoleFieldPermissions WHERE 1 = 1";
            ...
            return new PagedResult<RoleFieldPermission>
            {
                Items = items.ToList(),
    ...
    public async Task<RoleFieldPermission?> GetByIdAsync(Guid id)
    {
        try
        {
            const string sql = @"
        ...
        {
            _logger.LogError($"查詢角色欄位權限失敗: {id}", ex);
            throw;
        }
    }

    public async Task<Guid> CreateAsync(RoleFieldPermission entity)
    {
        try
        {
            var id = Guid.NewGuid();
            const string sql = @"
                INSERT INTO RoleFieldPermissions (
                    Id,
                    RoleId,
                    DbName,
                    TableName,
    ...
    public async Task<bool> UpdateAsync(RoleFieldPermission entity)
    {
        try
        {
            const string sql = @"
        ...
        {
            _logger.LogError($"修改角色欄位權限失敗: {entity.Id}", ex);
            throw;
        }
    }
    ...
    public async Task<bool> DeleteAsync(Guid id)
    {
        try
        {
            const string sql = "DELETE FROM RoleFieldPermissions WHERE Id = @Id";
    ...
    public async Task<int> BatchCreateAsync(List<RoleFieldPermission> entities)
    {
        try
        {
        ...
        {
            _logger.LogError("批次新增角色欄位權限失敗", ex);
            throw;
        }
    }
    ...
    public async Task<int> BatchDeleteAsync(List<Guid> ids)
    {
        try
        {
            ...
            const string sql = "DELETE FROM RoleFieldPermissions WHERE Id IN @Ids";
        ...
        {
            _logger.LogError("批次刪除角色欄位權限失敗", ex);
            throw;
        }
    }
}
