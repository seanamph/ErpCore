...
/// MIRW000 模組控制器
/// </summary>
[Route("api/v1/mirw000")]
public class MirW000Controller : BaseController
{
    private readonly IMirW000DataService _dataService;

    public MirW000Controller(
        IMirW000DataService dataService,
    ...
    /// </summary>
    [HttpGet("data")]
    public async Task<ActionResult<ApiResponse<PagedResult<MirW000DataDto>>>> GetDataList(
        [FromQuery] MirW000DataQueryDto query)
    {
    ...
    /// </summary>
    [HttpGet("data/{dataId}")]
    public async Task<ActionResult<ApiResponse<MirW000DataDto>>> GetData(string dataId)
    {
        return await ExecuteAsync(async () =>
        {
            var result = await _dataService.GetDataByIdAsync(dataId);
            return result;
        }, $"查詢MIRW000資料失敗: {dataId}");
    }
    ...
    public async Task<ActionResult<ApiResponse<string>>> CreateData(
        [FromBody] CreateMirW000DataDto dto)
    {
        return await ExecuteAsync(async () =>
        {
    ...
    public async Task<ActionResult<ApiResponse<object>>> UpdateData(
        string dataId,
        [FromBody] UpdateMirW000DataDto dto)
    {
        return await ExecuteAsync(async () =>
        {
            await _dataService.UpdateDataAsync(dataId, dto);
            return new object();
        }, $"修改MIRW000資料失敗: {dataId}");
    }
