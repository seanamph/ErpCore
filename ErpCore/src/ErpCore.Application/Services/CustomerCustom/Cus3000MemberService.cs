...
/// <summary>
/// CUS3000 會員服務實作 (SYS3130-SYS3160 - 會員管理)
/// </summary>
public class Cus3000MemberService : BaseService, ICus3000MemberService
{
    private readonly ICus3000MemberRepository _repository;

    public Cus3000MemberService(
        ICus3000MemberRepository repository,
    ...
    public async Task<PagedResult<Cus3000MemberDto>> GetCus3000MemberListAsync(Cus3000MemberQueryDto query)
    {
        try
        {
            var repositoryQuery = new Cus3000MemberQuery
            {
            ...
            return new PagedResult<Cus3000MemberDto>
            {
                Items = dtos,
    ...
    public async Task<Cus3000MemberDto?> GetCus3000MemberByIdAsync(long tKey)
    {
        try
        {
    ...
    public async Task<Cus3000MemberDto?> GetCus3000MemberByMemberIdAsync(string memberId)
    {
        try
        {
    ...
    public async Task<Cus3000MemberDto?> GetCus3000MemberByCardNoAsync(string cardNo)
    {
        try
        {
    ...
    public async Task<long> CreateCus3000MemberAsync(CreateCus3000MemberDto dto)
    {
        try
        {
            // 檢查會員編號是否已存在
            ...
            _logger.LogInfo($"新增CUS3000會員成功: {dto.MemberId}");
            return tKey;
        }
        catch (Exception ex)
        {
            _logger.LogError("新增CUS3000會員失敗", ex);
            throw;
        }
    }

    public async Task UpdateCus3000MemberAsync(long tKey, UpdateCus3000MemberDto dto)
    {
        try
        {
            var entity = await _repository.GetByIdAsync(tKey);
            ...
            _logger.LogInfo($"修改CUS3000會員成功: {tKey}");
        }
        catch (Exception ex)
        {
            _logger.LogError($"修改CUS3000會員失敗: {tKey}", ex);
            throw;
        }
    }

    public async Task DeleteCus3000MemberAsync(long tKey)
    {
        try
        {
            var entity = await _repository.GetByIdAsync(tKey);
            ...
            _logger.LogInfo($"刪除CUS3000會員成功: {tKey}");
        }
        catch (Exception ex)
        {
    ...
    private Cus3000MemberDto MapToDto(Cus3000Member entity)
    {
        return new Cus3000MemberDto
        {
            TKey = entity.TKey,
