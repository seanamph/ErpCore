<template>
  <div class="transfer-return">
    ...
    <!-- 待驗退調撥單選擇對話框 -->
    <el-dialog
      v-model="pendingTransferDialogVisible"
      title="選擇待驗退調撥單"
      width="80%"
      :close-on-click-modal="false"
    >
      <el-form :inline="true" :model="pendingTransferQueryForm" class="search-form">
        <el-form-item label="調撥單號">
          <el-input v-model="pendingTransferQueryForm.TransferId" placeholder="請輸入調撥單號" clearable />
        </el-form-item>
        <el-form-item label="調出分店">
          <el-input v-model="pendingTransferQueryForm.FromShopId" placeholder="請輸入調出分店" clearable />
        </el-form-item>
        <el-form-item label="調入分店">
          <el-input v-model="pendingTransferQueryForm.ToShopId" placeholder="請輸入調入分店" clearable />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="handlePendingTransferSearch">查詢</el-button>
          <el-button @click="handlePendingTransferReset">重置</el-button>
        </el-form-item>
      </el-form>

      <el-table
        :data="pendingTransferTableData"
        v-loading="pendingTransferLoading"
        border
        stripe
        style="width: 100%; margin-top: 20px"
        @row-click="handlePendingTransferSelect"
      ...
      >
        ...
        <el-table-column prop="ReturnQty" label="已驗退數量" width="120" />
        <el-table-column prop="PendingReturnQty" label="待驗退數量" width="120" />
      </el-table>

      <el-pagination
        v-model:current-page="pendingTransferPagination.PageIndex"
        v-model:page-size="pendingTransferPagination.PageSize"
        :page-sizes="[10, 20, 50, 100]"
        :total="pendingTransferPagination.TotalCount"
        layout="total, sizes, prev, pager, next, jumper"
        @size-change="handlePendingTransferSizeChange"
        @current-change="handlePendingTransferPageChange"
        style="margin-top: 20px; text-align: right;"
      />

      <template #footer>
        <el-button @click="pendingTransferDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handlePendingTransferConfirm" :disabled="!selectedPendingTransfer">確定</el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      >
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="驗收單號" prop="ReceiptId">
              <el-input v-model="formData.ReceiptId" :disabled="isEdit" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
...
export default {
  ...
  setup() {
    ...
    const handleCreateFromPending = async () => {
      try {
        pendingTransferDialogVisible.value = true
        selectedPendingTransfer.value = null
        Object.assign(pendingTransferQueryForm, {
          TransferId: '',
          FromShopId: '',
          ToShopId: '',
          PageIndex: 1,
    ...
    const loadPendingTransfers = async () => {
      pendingTransferLoading.value = true
      try {
        const params = {
          ...pendingTransferQueryForm,
          PageIndex: pendingTransferPagination.PageIndex,
          PageSize: pendingTransferPagination.PageSize
        }
        const response = await transferReturnApi.getPendingTransfers(params)
        if (response.Data) {
          pendingTransferTableData.value = response.Data.Items || []
          pendingTransferPagination.TotalCount = response.Data.TotalCount || 0
        }
      } catch (error) {
        ElMessage.error('查詢待驗退調撥單失敗: ' + (error.message || '未知錯誤'))
      } finally {
        pendingTransferLoading.value = false
      }
    }
    
    // 待驗退調撥單查詢
    const handlePendingTransferSearch = () => {
      pendingTransferPagination.PageIndex = 1
      loadPendingTransfers()
    }
    
    // 待驗退調撥單重置
    const handlePendingTransferReset = () => {
      Object.assign(pendingTransferQueryForm, {
        TransferId: '',
        FromShopId: '',
        ToShopId: ''
    ...
    // 選擇待驗退調撥單
    const handlePendingTransferSelect = (row) => {
      selectedPendingTransfer.value = row
    }
    
    // 確認選擇待驗退調撥單
    const handlePendingTransferConfirm = async () => {
      if (!selectedPendingTransfer.value) {
        ElMessage.warning('請選擇一筆調撥單')
        return
      }
      
      try {
        const response = await transferReturnApi.createReturnFromTransfer(selectedPendingTransfer.value.TransferId)
        if (response.Data) {
          isEdit.value = true
          dialogVisible.value = true
          pendingTransferDialogVisible.value = false
    ...
    // 待驗退調撥單分頁大小變更
    const handlePendingTransferSizeChange = (size) => {
      pendingTransferPagination.PageSize = size
      loadPendingTransfers()
    }
    
    // 待驗退調撥單分頁變更
    const handlePendingTransferPageChange = (page) => {
      pendingTransferPagination.PageIndex = page
      loadPendingTransfers()
    }
    ...
    const handleCreateFromTransfer = async () => {
      try {
        const { value: transferId } = await ElMessageBox.prompt('請輸入調撥單號', '依調撥單建立驗退單', {
        ...
        const response = await transferReturnApi.createReturnFromTransfer(transferId)
        if (response.Data) {
          isEdit.value = true
          dialogVisible.value = true
          Object.assign(formData, response.Data)
    ...
    const handleSubmit = async () => {
      ...
      await formRef.value.validate(async (valid) => {
        if (valid) {
          try {
            // 計算總金額和總數量
            const totalQty = formData.Details.reduce((sum, item) => sum + (item.ReturnQty || 0), 0)
            const totalAmount = formData.Details.reduce((sum, item) => sum + ((item.UnitPrice || 0) * (item.ReturnQty || 0)), 0)
