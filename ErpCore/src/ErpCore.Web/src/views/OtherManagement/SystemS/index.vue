...
import { systemSApi } from '@/api/otherManagement'
...
const dialogTitle = computed(() => isEdit.value ? '修改S系統功能' : '新增S系統功能')
...
const handleSearch = async () => {
  ...
  try {
    ...
    const response = await systemSApi.getSystemSList(params)
...
const handleEdit = async (row) => {
  try {
    const response = await systemSApi.getSystemS(row.TKey)
    if (response.data?.success) {
      isEdit.value = true
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('確定要刪除此S系統功能嗎？', '確認', {
      type: 'warning'
    })
    const response = await systemSApi.deleteSystemS(row.TKey)
    if (response.data?.success) {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        let response
        if (isEdit.value) {
          response = await systemSApi.updateSystemS(currentTKey.value, formData)
        } else {
          response = await systemSApi.createSystemS(formData)
        }
        if (response.data?.success) {
