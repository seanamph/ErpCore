...
import { systemVApi } from '@/api/otherManagement'
...
const dialogTitle = computed(() => isEdit.value ? '修改V系統功能' : '新增V系統功能')
...
const handleSearch = async () => {
  ...
  try {
    ...
    const response = await systemVApi.getSystemVList(params)
...
const handleEdit = async (row) => {
  try {
    const response = await systemVApi.getSystemV(row.TKey)
    if (response.data?.success) {
      isEdit.value = true
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('確定要刪除此V系統功能嗎？', '確認', {
      type: 'warning'
    })
    const response = await systemVApi.deleteSystemV(row.TKey)
    if (response.data?.success) {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        let response
        if (isEdit.value) {
          response = await systemVApi.updateSystemV(currentTKey.value, formData)
        } else {
          response = await systemVApi.createSystemV(formData)
        }
        if (response.data?.success) {
