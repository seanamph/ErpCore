  ...
  <div class="system-u">
    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm" class="search-form">
        <el-form-item label="功能代碼">
          <el-input v-model="queryForm.FunctionCode" placeholder="請輸入功能代碼" clearable />
        </el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="FunctionCode" label="功能代碼" width="150" />
    ...
    >
      <el-form
        ...
        label-width="150px"
      >
        <el-form-item label="功能代碼" prop="FunctionCode">
          <el-input v-model="formData.FunctionCode" :disabled="isEdit" />
        </el-form-item>
...
import { systemUApi } from '@/api/otherManagement'
...
const dialogTitle = computed(() => isEdit.value ? '修改U系統功能' : '新增U系統功能')
...
const formRules = {
  FunctionCode: [{ required: true, message: '請輸入功能代碼', trigger: 'blur' }],
...
const handleSearch = async () => {
  ...
  try {
    const params = {
      ...
      FunctionName: queryForm.FunctionName || undefined
    }
    const response = await systemUApi.getSystemUList(params)
...
const handleEdit = async (row) => {
  try {
    const response = await systemUApi.getSystemU(row.TKey)
    if (response.data?.success) {
      isEdit.value = true
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('確定要刪除此U系統功能嗎？', '確認', {
      type: 'warning'
    })
    const response = await systemUApi.deleteSystemU(row.TKey)
    if (response.data?.success) {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        let response
        if (isEdit.value) {
          response = await systemUApi.updateSystemU(currentTKey.value, formData)
        } else {
          response = await systemUApi.createSystemU(formData)
        }
        if (response.data?.success) {
