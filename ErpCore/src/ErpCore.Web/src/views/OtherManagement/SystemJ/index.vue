...
import { systemJApi } from '@/api/otherManagement'
...
const dialogTitle = computed(() => isEdit.value ? '修改J系統功能' : '新增J系統功能')
...
const handleSearch = async () => {
  ...
  try {
    ...
    const response = await systemJApi.getSystemJList(params)
...
const handleEdit = async (row) => {
  try {
    const response = await systemJApi.getSystemJ(row.TKey)
    if (response.data?.success) {
      isEdit.value = true
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('確定要刪除此J系統功能嗎？', '確認', {
      type: 'warning'
    })
    const response = await systemJApi.deleteSystemJ(row.TKey)
    if (response.data?.success) {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        let response
        if (isEdit.value) {
          response = await systemJApi.updateSystemJ(currentTKey.value, formData)
        } else {
          response = await systemJApi.createSystemJ(formData)
        }
        if (response.data?.success) {
