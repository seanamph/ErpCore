...
import { purchaseReceiptV2Api } from '@/api/purchase'
  ...
  setup() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const response = await purchaseReceiptV2Api.getPurchaseReceipts(params)
        if (response.Data) {
    ...
    const handleCreateFromOrder = async () => {
      try {
        ...
        const response = await purchaseReceiptV2Api.createReceiptFromOrder(orderId)
    ...
    const handleView = async (row) => {
      try {
        const response = await purchaseReceiptV2Api.getPurchaseReceipt(row.ReceiptId)
        if (response.Data && response.Data.Receipt) {
          isEdit.value = true
          dialogVisible.value = true
          const receipt = response.Data.Receipt
    ...
    const handleConfirm = async (row) => {
      try {
        ...
        await purchaseReceiptV2Api.confirmReceipt(row.ReceiptId)
    ...
    const handleCancel = async (row) => {
      try {
        ...
        await purchaseReceiptV2Api.cancelPurchaseReceipt(row.ReceiptId)
    ...
    const handleDelete = async (row) => {
      try {
        ...
        await purchaseReceiptV2Api.deletePurchaseReceipt(row.ReceiptId)
      ...
      await formRef.value.validate(async (valid) => {
        if (valid) {
          try {
            ...
            if (isEdit.value) {
              await purchaseReceiptV2Api.updatePurchaseReceipt(formData.ReceiptId, {
              ...
              await purchaseReceiptV2Api.createPurchaseReceipt({
