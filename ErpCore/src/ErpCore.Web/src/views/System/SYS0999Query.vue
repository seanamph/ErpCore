    ...
    <el-card class="search-card" shadow="never">
      <el-form :model="queryForm" label-width="120px">
        ...
        <el-form-item>
          ...
          <el-button type="success" @click="handleExportExcel" :disabled="!queryForm.SystemId">匯出Excel（出庫用）</el-button>
          <el-button type="warning" @click="handleExportPdf" :disabled="!queryForm.SystemId">匯出PDF（出庫用）</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  ...
  setup() {
    ...
    const handleExportExcel = async () => {
      ...
      try {
        ...
        link.href = url
        link.download = `系統作業與功能列表_出庫用_${queryForm.SystemId}_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.xlsx`
    ...
    const handleExportPdf = async () => {
      ...
      try {
        ...
        const link = document.createElement('a')
        link.href = url
        link.download = `系統作業與功能列表_出庫用_${queryForm.SystemId}_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.pdf`
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
        window.URL.revokeObjectURL(url)
