<template>
  <div class="system-program-button-export-query">
    <div class="page-header">
      <h1>系統作業與功能列表查詢（出庫用）(SYS0999)</h1>
    </div>
...
import { getSystemProgramButtonsForExport, exportSystemProgramButtonsReport } from '@/api/systems'

export default {
  name: 'SystemProgramButtonExportQuery',
  setup() {
    ...
    const loadData = async () => {
      ...
      loading.value = true
      try {
        const response = await getSystemProgramButtonsForExport(queryForm.SystemId)
        // 處理不同的響應格式
        let data = null
        if (response && response.data) {
          if (response.data.success && response.data.data) {
    ...
    const handleExportExcel = async () => {
      ...
      try {
        loading.value = true
        const response = await exportSystemProgramButtonsReport(queryForm.SystemId, 'Excel')
        ...
        link.href = url
        link.download = `系統作業與功能列表（出庫用）_${queryForm.SystemId}_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.xlsx`
    ...
    const handleExportPdf = async () => {
      ...
      try {
        loading.value = true
        const response = await exportSystemProgramButtonsReport(queryForm.SystemId, 'PDF')
        ...
        const link = document.createElement('a')
        link.href = url
        link.download = `系統作業與功能列表（出庫用）_${queryForm.SystemId}_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.pdf`
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
        window.URL.revokeObjectURL(url)
...
.system-program-button-export-query {
  .search-card {
    margin-bottom: 20px;
  }
