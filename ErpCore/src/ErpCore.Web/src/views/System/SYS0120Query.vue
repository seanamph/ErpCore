<template>
  <div class="sys0120-query">
    <div class="page-header">
      <h1>使用者資料瀏覽 (SYS0120)</h1>
    </div>
    ...
    >
      <el-descriptions :column="2" border v-if="detailData">
        ...
        <el-descriptions-item label="有效起始日">{{ detailData.StartDate || '-' }}</el-descriptions-item>
        <el-descriptions-item label="有效終止日">{{ detailData.EndDate || '-' }}</el-descriptions-item>
...
import { browseUsers, browseUserById } from '@/api/users'
...
const handleView = async (row) => {
  try {
    loading.value = true
    const response = await browseUserById(row.UserId)
    if (response && response.Data) {
