<template>
  <div class="sys0330-query">
    <div class="page-header">
      <h1>角色欄位權限設定 (SYS0330)</h1>
    </div>
    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm" class="search-form">
        ...
        <el-form-item>
          ...
          <el-button type="success" @click="handleAdd" :disabled="!queryForm.RoleId">新增</el-button>
          <el-button type="warning" @click="handleBatchSet" :disabled="!queryForm.RoleId">批量設定</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="RoleName" label="角色" width="150" sortable />
    ...
    >
      <el-form :model="form" :rules="formRules" ref="formRef" label-width="120px">
        <el-form-item label="角色" prop="RoleId">
          <el-select v-model="form.RoleId" placeholder="請選擇角色" filterable style="width: 100%" :disabled="isEdit">
            <el-option
              v-for="item in roleOptions"
              :key="item.RoleId"
              :label="item.RoleName"
    ...
    >
      <el-form :model="batchForm" label-width="120px">
        <el-form-item label="角色" required>
          <el-select v-model="batchForm.RoleId" placeholder="請選擇角色" filterable style="width: 100%">
            <el-option
              v-for="item in roleOptions"
              :key="item.RoleId"
              :label="item.RoleName"
        ...
        <el-form-item label="欄位列表">
          <el-table :data="fieldOptions" border stripe style="width: 100%" max-height="400">
            <el-table-column type="selection" width="55" align="center" />
            <el-table-column type="index" label="序號" width="60" align="center" />
...
import { roleFieldPermissionsApi } from '@/api/permissions'
...
const dialogTitle = ref('新增角色欄位權限')
...
const loadDbOptions = async () => {
  try {
    const response = await roleFieldPermissionsApi.getDatabases()
    if (response && response.Data) {
...
const loadTableOptions = async (dbName) => {
  ...
  try {
    const response = await roleFieldPermissionsApi.getTables(dbName)
    if (response && response.Data) {
...
const loadFieldOptions = async (dbName, tableName) => {
  ...
  try {
    const response = await roleFieldPermissionsApi.getFields(dbName, tableName)
    if (response && response.Data) {
...
const loadData = async () => {
  ...
  try {
    ...
    const response = await roleFieldPermissionsApi.getRoleFieldPermissions(params)
    if (response && response.Data) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await roleFieldPermissionsApi.updateRoleFieldPermission(currentId.value, {
        PermissionType: form.PermissionType
      })
      ElMessage.success('修改成功')
    } else {
      await roleFieldPermissionsApi.createRoleFieldPermission({
        RoleId: form.RoleId,
        DbName: form.DbName,
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(`確定要刪除角色「${row.RoleName}」對欄位「${row.FieldName}」的權限設定嗎？`, '確認刪除', {
      type: 'warning'
    })
    await roleFieldPermissionsApi.deleteRoleFieldPermission(row.Id)
...
const handleBatchSet = () => {
  ...
  batchForm.RoleId = queryForm.RoleId
  batchForm.DbName = ''
  batchForm.TableName = ''
...
const handleBatchSubmit = async () => {
  if (!batchForm.RoleId || !batchForm.DbName || !batchForm.TableName || !batchForm.PermissionType) {
  ...
  try {
    await ElMessageBox.confirm('確定要批量設定角色欄位權限嗎？', '確認設定', {
      type: 'warning'
    })
    const permissions = fieldOptions.value.map(field => ({
      DbName: batchForm.DbName,
      TableName: batchForm.TableName,
      FieldName: field.FieldName,
      PermissionType: batchForm.PermissionType
    }))
    await roleFieldPermissionsApi.batchSetPermissions({
