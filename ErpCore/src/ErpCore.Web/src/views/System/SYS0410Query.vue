    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm" class="search-form">
        ...
        <el-form-item label="系統型態">
          <el-select v-model="queryForm.SystemType" placeholder="請選擇系統型態" clearable multiple style="width: 200px">
            <el-option
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="150px">
        ...
        <el-form-item label="資料庫密碼" prop="DbPass">
          <el-input v-model="form.DbPass" type="password" placeholder="請輸入資料庫密碼" show-password maxlength="255" />
        </el-form-item>
...
const loadSystemTypes = async () => {
  try {
    ...
    if (response && response.Data) {
      systemTypeOptions.value = response.Data.map(item => ({
        Value: item.Value,
        Label: item.Label
      }))
    }
  ...
  try {
    const params = {
      ...
      Filters: {
        ...
        SystemType: queryForm.SystemType && queryForm.SystemType.length > 0 ? queryForm.SystemType.join(',') : undefined,
  ...
  try {
    ...
    if (response && response.Data) {
      Object.assign(form, {
        ...
        SystemType: response.Data.SystemType || '',
        ServerIp: response.Data.ServerIp || '',
        ModuleId: response.Data.ModuleId || '',
        DbUser: response.Data.DbUser || '',
        DbPass: '', // 不顯示密碼
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(`確定要刪除主系統「${row.SystemName}」嗎？`, '確認刪除', {
      type: 'warning'
    })
    await deleteSystem(row.SystemId)
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    await deleteSystemsBatch({ SystemIds: systemIds })
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      ...
      await updateSystem(form.SystemId, updateData)
