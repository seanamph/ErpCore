<template>
  <div class="sys0660-query">
    <div class="page-header">
      <h1>其他異動記錄查詢 (SYS0660)</h1>
    </div>
          ...
          >
            <template #title>
              <div class="collapse-title">
                ...
                <span class="program-id">作業代碼: <strong>{{ item.ProgramId || '-' }}</strong></span>
                <span class="program-name" v-if="item.ProgramName">作業名稱: <strong>{{ item.ProgramName }}</strong></span>
...
const handleQuery = async () => {
  ...
  await queryFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const response = await changeLogsApi.getOtherChangeLogs(queryData)
      ...
      console.error('查詢其他異動記錄失敗:', error)
