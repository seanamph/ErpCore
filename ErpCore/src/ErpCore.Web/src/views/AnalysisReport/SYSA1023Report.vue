<template>
  <div class="sysa1023-report">
    <div class="page-header">
      <h1>工務維修統計報表(報表類型) (SYSA1023)</h1>
    </div>
      ...
      <el-form :inline="true" :model="queryForm" class="search-form">
        <el-form-item label="報表類型">
          ...
          >
            ...
            <el-option label="類型1" value="1" />
            <el-option label="類型2" value="2" />
            <!-- 可以根據實際需求添加更多選項 -->
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async handleSearch() {
      ...
      this.loading = true
      try {
        const response = await analysisReportApi.getSYSA1023Report(this.queryForm)
        ...
        console.error('查詢工務維修統計報表(報表類型)失敗:', error)
        ElMessage.error('查詢工務維修統計報表(報表類型)失敗')
      } finally {
        this.loading = false
      }
    },
    ...
    async handleExport() {
      ...
      try {
        const response = await analysisReportApi.exportSYSA1023Report(this.queryForm, 'excel')
        ...
        link.href = url
        link.download = `SYSA1023_工務維修統計報表(報表類型)_${new Date().toISOString().slice(0, 10)}.xlsx`
        link.click()
        window.URL.revokeObjectURL(url)
        ElMessage.success('匯出成功')
      } catch (error) {
        console.error('匯出工務維修統計報表(報表類型)失敗:', error)
        ElMessage.error('匯出工務維修統計報表(報表類型)失敗')
      }
    },
    async handlePrint() {
      if (!this.queryForm.ApplyDateB || !this.queryForm.ApplyDateE) {
      ...
      try {
        const response = await analysisReportApi.printSYSA1023Report(this.queryForm)
        const blob = new Blob([response.data], { type: 'application/pdf' })
        const url = window.URL.createObjectURL(blob)
        window.open(url, '_blank')
        ElMessage.success('列印成功')
      } catch (error) {
        console.error('列印工務維修統計報表(報表類型)失敗:', error)
        ElMessage.error('列印工務維修統計報表(報表類型)失敗')
      }
    },
