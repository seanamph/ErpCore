<template>
  <div class="sysa1022-report">
    <div class="page-header">
      <h1>工務維修統計報表 (SYSA1022)</h1>
    </div>
...
export default {
  ...
  methods: {
    ...
    async handleSearch() {
      ...
      this.loading = true
      try {
        const response = await analysisReportApi.getSYSA1022Report(this.queryForm)
        ...
        console.error('查詢工務維修統計報表失敗:', error)
        ElMessage.error('查詢工務維修統計報表失敗')
      } finally {
        this.loading = false
      }
    },
    ...
    async handleExport() {
      ...
      try {
        const response = await analysisReportApi.exportSYSA1022Report(this.queryForm, 'excel')
        ...
        link.href = url
        link.download = `SYSA1022_工務維修統計報表_${new Date().toISOString().slice(0, 10)}.xlsx`
        link.click()
        window.URL.revokeObjectURL(url)
        ElMessage.success('匯出成功')
      } catch (error) {
        console.error('匯出工務維修統計報表失敗:', error)
        ElMessage.error('匯出工務維修統計報表失敗')
      }
    },
    async handlePrint() {
      if (!this.queryForm.ApplyDateB || !this.queryForm.ApplyDateE) {
      ...
      try {
        const response = await analysisReportApi.printSYSA1022Report(this.queryForm)
        const blob = new Blob([response.data], { type: 'application/pdf' })
        const url = window.URL.createObjectURL(blob)
        window.open(url, '_blank')
        ElMessage.success('列印成功')
      } catch (error) {
        console.error('列印工務維修統計報表失敗:', error)
        ElMessage.error('列印工務維修統計報表失敗')
      }
    },
