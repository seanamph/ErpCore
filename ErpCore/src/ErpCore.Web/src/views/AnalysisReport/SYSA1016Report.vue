<template>
  <div class="sysa1016-report">
    <div class="page-header">
      <h1>商品分析報表 (SYSA1016)</h1>
    </div>
...
export default {
  ...
  methods: {
    ...
    async handleSearch() {
      this.loading = true
      try {
        const response = await analysisReportApi.getSYSA1016Report(this.queryForm)
    ...
    async handleExport() {
      try {
        const response = await analysisReportApi.exportSYSA1016Report(this.queryForm, 'excel')
    ...
    async handlePrint() {
      try {
        const response = await analysisReportApi.printSYSA1016Report(this.queryForm)
