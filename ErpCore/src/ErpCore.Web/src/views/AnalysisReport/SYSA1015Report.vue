<template>
  <div class="sysa1015-report">
    <div class="page-header">
      <h1>商品分析報表 (SYSA1015)</h1>
    </div>
...
export default {
  ...
  methods: {
    ...
    async handleSearch() {
      this.loading = true
      try {
        const response = await analysisReportApi.getSYSA1015Report(this.queryForm)
    ...
    async handleExport() {
      try {
        const response = await analysisReportApi.exportSYSA1015Report(this.queryForm, 'excel')
        ...
        link.href = url
        link.download = `商品分析報表_${this.queryForm.YearMonth || '全部'}_${new Date().toISOString().slice(0, 10)}.xlsx`
        link.click()
    ...
    async handlePrint() {
      try {
        const response = await analysisReportApi.printSYSA1015Report(this.queryForm)
