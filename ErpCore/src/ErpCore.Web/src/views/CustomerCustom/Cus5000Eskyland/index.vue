<template>
  <div class="cus5000-eskyland">
    <div class="page-header">
      <h1>CUS5000.ESKYLAND 客戶定制模組</h1>
    </div>
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm" class="search-form">
        <el-form-item label="資料編號">
          <el-input v-model="queryForm.DataId" placeholder="請輸入資料編號" clearable />
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px" :close-on-click-modal="false">
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="120px">
        <el-form-item label="資料編號" prop="DataId">
          <el-input v-model="formData.DataId" :disabled="isEdit" placeholder="請輸入資料編號" />
        </el-form-item>
        <el-form-item label="資料名稱" prop="DataName">
          <el-input v-model="formData.DataName" placeholder="請輸入資料名稱" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const api = {
  getList: (params) => axios.get('/api/v1/cus5000-eskyland/data', { params }),
  getById: (tKey) => axios.get(`/api/v1/cus5000-eskyland/data/${tKey}`),
  create: (data) => axios.post('/api/v1/cus5000-eskyland/data', data),
  update: (tKey, data) => axios.put(`/api/v1/cus5000-eskyland/data/${tKey}`, data),
  delete: (tKey) => axios.delete(`/api/v1/cus5000-eskyland/data/${tKey}`)
}
...
const formRules = {
  DataId: [{ required: true, message: '請輸入資料編號', trigger: 'blur' }],
  DataName: [{ required: true, message: '請輸入資料名稱', trigger: 'blur' }]
}
