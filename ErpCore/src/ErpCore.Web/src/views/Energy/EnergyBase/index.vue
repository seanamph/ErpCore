<template>
  <div class="energy-base-management">
    <div class="page-header">
      <h1>?ΩÊ??∫Á?Á∂≠Ë≠∑ (SYSO100-SYSO130)</h1>
    </div>
    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm" class="search-form">
        ...
        <el-form-item label="?ΩÊ??çÁ®±">
          <el-input v-model="queryForm.EnergyName" placeholder="Ë´ãËº∏?•ËÉΩÊ∫êÂ?Á®? clearable />
        </el-form-item>
        <el-form-item label="?ΩÊ?È°ûÂ?">
          <el-select v-model="queryForm.EnergyType" placeholder="Ë´ãÈÅ∏?áËÉΩÊ∫êÈ??? clearable>
            <el-option label="?®ÈÉ®" value="" />
            <el-option label="?ªÂ?" value="ELECTRICITY" />
            <el-option label="Â§©ÁÑ∂Ê∞? value="GAS" />
            <el-option label="Ê∞? value="WATER" />
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="EnergyName" label="?ΩÊ??çÁ®±" width="200" />
        <el-table-column prop="EnergyType" label="?ΩÊ?È°ûÂ?" width="120">
          <template #default="{ row }">
            <el-tag :type="getEnergyTypeTag(row.EnergyType)">
              {{ getEnergyTypeText(row.EnergyType) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="Unit" label="?Æ‰?" width="100" />
    ...
    >
      ...
      >
        <el-form-item label="?ΩÊ?Á∑®Ë?" prop="EnergyId">
          <el-input v-model="formData.EnergyId" :disabled="isEdit" placeholder="Ë´ãËº∏?•ËÉΩÊ∫êÁ∑®?? />
        </el-form-item>
        <el-form-item label="?ΩÊ??çÁ®±" prop="EnergyName">
          <el-input v-model="formData.EnergyName" placeholder="Ë´ãËº∏?•ËÉΩÊ∫êÂ?Á®? />
        </el-form-item>
        <el-form-item label="?ΩÊ?È°ûÂ?" prop="EnergyType">
          <el-select v-model="formData.EnergyType" placeholder="Ë´ãÈÅ∏?áËÉΩÊ∫êÈ??? style="width: 100%">
            <el-option label="?ªÂ?" value="ELECTRICITY" />
            <el-option label="Â§©ÁÑ∂Ê∞? value="GAS" />
            <el-option label="Ê∞? value="WATER" />
            <el-option label="?∂‰?" value="OTHER" />
        ...
        <el-form-item label="?Æ‰?" prop="Unit">
          <el-input v-model="formData.Unit" placeholder="Ë´ãËº∏?•ÂñÆ‰Ω? />
        </el-form-item>
...
export default {
  ...
  setup() {
    ...
    const formRules = {
      ...
      EnergyName: [{ required: true, message: 'Ë´ãËº∏?•ËÉΩÊ∫êÂ?Á®?, trigger: 'blur' }]
    }

    // Â∞çË©±Ê°ÜÊ?È°?
    const dialogTitle = computed(() => {
      return isEdit.value ? '‰øÆÊîπ?ΩÊ??∫Á?Ë≥áÊ?' : '?∞Â??ΩÊ??∫Á?Ë≥áÊ?'
    })
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const response = await energyApi.getEnergyBases(params)
        ...
        console.error('?•Ë©¢?ΩÊ??∫Á??óË°®Â§±Ê?:', error)
    ...
    const handleView = async (row) => {
      try {
        const response = await energyApi.getEnergyBase(row.TKey)
        if (response.data && response.data.Success) {
          isEdit.value = true
        ...
        console.error('?•Ë©¢?ΩÊ??∫Á?Ë≥áÊ?Â§±Ê?:', error)
        ElMessage.error('?•Ë©¢Â§±Ê?: ' + (error.message || '?™Áü•?ØË™§'))
      }
    }
    ...
    const handleDelete = async (row) => {
      try {
        await ElMessageBox.confirm('Á¢∫Â?Ë¶ÅÂà™?§Ê≠§?ΩÊ??∫Á?Ë≥áÊ??éÔ?', 'Á¢∫Ë?', {
          type: 'warning'
        })
        await energyApi.deleteEnergyBase(row.TKey)
    ...
    const handleSubmitForm = async () => {
      ...
      try {
        ...
        if (isEdit.value) {
          await energyApi.updateEnergyBase(currentTKey.value, formData)
          ElMessage.success('‰øÆÊîπ?êÂ?')
        } else {
          await energyApi.createEnergyBase(formData)
