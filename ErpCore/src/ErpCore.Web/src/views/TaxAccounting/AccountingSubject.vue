<template>
  <div class="tax-accounting-subject">
    <div class="page-header">
      <h1>會計科目維護 (SYST111-SYST11A)</h1>
    </div>
  ...
  setup() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const response = await taxAccountingApi.getAccountSubjects(params)
        if (response.Data) {
    ...
    const handleView = async (row) => {
      try {
        const response = await taxAccountingApi.getAccountSubject(row.StypeId)
        if (response.Data) {
          isEdit.value = true
          dialogVisible.value = true
    ...
    const handleDelete = async (row) => {
      try {
        ...
        await taxAccountingApi.deleteAccountSubject(row.StypeId)
    ...
    const handleSubmit = async () => {
      ...
      await formRef.value.validate(async (valid) => {
        if (valid) {
          try {
            if (isEdit.value) {
              await taxAccountingApi.updateAccountSubject(formData.StypeId, formData)
              ElMessage.success('修改成功')
            } else {
              await taxAccountingApi.createAccountSubject(formData)
