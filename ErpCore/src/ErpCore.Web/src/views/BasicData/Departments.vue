    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm" class="search-form">
        <el-form-item label="部別代碼">
          <el-input v-model="queryForm.DeptId" placeholder="請輸入部別代碼" clearable />
        </el-form-item>
        <el-form-item label="部別名稱">
          <el-input v-model="queryForm.DeptName" placeholder="請輸入部別名稱" clearable />
        </el-form-item>
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        let response
        if (isEdit.value) {
          response = await departmentsApi.updateDepartment(form.DeptId, form)
        } else {
          response = await departmentsApi.createDepartment(form)
        }
        if (response.data.success) {
