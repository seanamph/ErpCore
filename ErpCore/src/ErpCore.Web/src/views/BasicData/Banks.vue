  ...
  <div class="banks">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="AcctLen" label="帳號最小長度" width="120" align="right" />
        <el-table-column prop="AcctLenMax" label="帳號最大長度" width="120" align="right" />
        ...
        <el-table-column prop="BankKind" label="銀行種類" width="120">
          <template #default="{ row }">
            {{ row.BankKind === '1' ? '銀行' : row.BankKind === '2' ? '郵局' : row.BankKind === '3' ? '信用合作社' : '' }}
          </template>
        </el-table-column>
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        ...
        <el-form-item label="帳號最小長度" prop="AcctLen">
          <el-input-number v-model="form.AcctLen" :min="0" :step="1" style="width: 100%" />
        </el-form-item>
        <el-form-item label="帳號最大長度" prop="AcctLenMax">
          <el-input-number v-model="form.AcctLenMax" :min="0" :step="1" style="width: 100%" />
        </el-form-item>
        ...
        <el-form-item label="銀行種類" prop="BankKind">
          <el-select v-model="form.BankKind" placeholder="請選擇銀行種類" style="width: 100%">
            <el-option label="銀行" value="1" />
            <el-option label="郵局" value="2" />
            <el-option label="信用合作社" value="3" />
          </el-select>
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        let response
        if (isEdit.value) {
          response = await banksApi.updateBank(form.BankId, form)
        } else {
