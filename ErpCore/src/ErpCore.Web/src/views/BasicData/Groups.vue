    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm" class="search-form">
        <el-form-item label="組別代碼">
          <el-input v-model="queryForm.GroupId" placeholder="請輸入組別代碼" clearable />
        </el-form-item>
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        <el-form-item label="組別代碼" prop="GroupId">
          <el-input v-model="form.GroupId" placeholder="請輸入組別代碼" :disabled="isEdit" />
        </el-form-item>
...
// 表單驗證規則
const rules = {
  GroupId: [{ required: true, message: '請輸入組別代碼', trigger: 'blur' }],
  GroupName: [{ required: true, message: '請輸入組別名稱', trigger: 'blur' }],
...
const handleEdit = async (row) => {
  isEdit.value = true
  dialogTitle.value = '修改組別'
  try {
    const response = await groupsApi.getGroup(row.GroupId)
    if (response.data.success) {
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('確定要刪除此組別嗎？', '提示', {
      type: 'warning'
    })
    const response = await groupsApi.deleteGroup(row.GroupId)
    if (response.data.success) {
