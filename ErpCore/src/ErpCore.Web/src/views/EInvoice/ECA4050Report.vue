<template>
  <div class="eca4050-report">
    <div class="page-header">
      <h1>電子發票報表查詢(出貨日期統計) (ECA4050)</h1>
    </div>
    ...
    <el-card class="table-card" shadow="never">
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="ShipDate" label="出貨日期" width="150" align="center">
          <template #default="{ row }">
            {{ formatDate(row.ShipDate) }}
          </template>
        </el-table-column>
        <el-table-column prop="OrderQty" label="銷售數量" width="120" align="right">
          <template #default="{ row }">
            {{ formatNumber(row.OrderQty) }}
          </template>
        </el-table-column>
        <el-table-column prop="OrderSubtotal" label="銷售金額" width="150" align="right">
          <template #default="{ row }">
            {{ formatCurrency(row.OrderSubtotal) }}
          </template>
        </el-table-column>
        <el-table-column prop="ShippingFee" label="運費" width="120" align="right">
          <template #default="{ row }">
            {{ formatCurrency(row.ShippingFee) }}
          </template>
        </el-table-column>
        <el-table-column prop="SalesPercent" label="銷售占比(%)" width="150" align="right">
          <template #default="{ row }">
  ...
  setup() {
    ...
    const handleSearch = async () => {
      try {
        ...
        if (dateRange.value && dateRange.value.length === 2) {
          queryForm.ShipDateFrom = dateRange.value[0]
          queryForm.ShipDateTo = dateRange.value[1]
        } else {
    ...
    const handleExportExcel = async () => {
      try {
        ...
        link.href = url
        link.download = `電子發票報表_出貨日期統計_${new Date().toISOString().slice(0, 10)}.xlsx`
        link.click()
    ...
    const handleExportPdf = async () => {
      try {
        ...
        const link = document.createElement('a')
        link.href = url
        link.download = `電子發票報表_出貨日期統計_${new Date().toISOString().slice(0, 10)}.pdf`
        link.click()
        window.URL.revokeObjectURL(url)
