<template>
  <div class="eca4020-report">
    <div class="page-header">
      <h1>電子發票報表查詢(商品銷售統計) (ECA4020)</h1>
    </div>
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="ScId" label="專櫃代碼" width="120" />
        <el-table-column prop="ScName" label="專櫃名稱" width="150" />
        ...
        <el-table-column prop="ProviderGoodsId" label="供應商商品編號" width="150" />
        ...
        <el-table-column prop="AvgPrice" label="平均價格" width="120" align="right">
          <template #default="{ row }">
...
export default {
  ...
  setup() {
    ...
    const avgPrice = computed(() => {
      if (totalQty.value > 0) {
        return totalAmount.value / totalQty.value
      }
      return 0
    })
    ...
    const handleExportExcel = async () => {
      try {
        ...
        link.href = url
        link.download = `電子發票報表_商品銷售統計_${new Date().toISOString().slice(0, 10)}.xlsx`
        link.click()
    ...
    const handleExportPdf = async () => {
      try {
        ...
        const link = document.createElement('a')
        link.href = url
        link.download = `電子發票報表_商品銷售統計_${new Date().toISOString().slice(0, 10)}.pdf`
        link.click()
        window.URL.revokeObjectURL(url)
