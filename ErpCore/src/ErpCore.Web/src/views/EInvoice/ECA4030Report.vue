<template>
  <div class="eca4030-report">
    <div class="page-header">
      <h1>電子發票報表查詢(零售商銷售統計) (ECA4030)</h1>
    </div>
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="RetailerId" label="零售商代碼" width="150" />
        <el-table-column prop="RetailerName" label="零售商名稱" width="200" show-overflow-tooltip />
  ...
  setup() {
    ...
    const handleExportExcel = async () => {
      try {
        ...
        link.href = url
        link.download = `電子發票報表_零售商銷售統計_${new Date().toISOString().slice(0, 10)}.xlsx`
        link.click()
    ...
    const handleExportPdf = async () => {
      try {
        ...
        const link = document.createElement('a')
        link.href = url
        link.download = `電子發票報表_零售商銷售統計_${new Date().toISOString().slice(0, 10)}.pdf`
        link.click()
        window.URL.revokeObjectURL(url)
