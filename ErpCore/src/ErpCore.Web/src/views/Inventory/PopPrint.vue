    ...
    >
      <el-form :model="settingsForm" ref="settingsFormRef" label-width="150px">
        ...
        <el-form-item label="報表類型">
          <el-input v-model="settingsForm.TypeId" placeholder="請輸入報表類型" />
        </el-form-item>
...
import { popPrintApi } from '@/api/popPrint'
  ...
  setup() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const response = await popPrintApi.getProducts(params)
        if (response.Success) {
      ...
      await printFormRef.value.validate(async (valid) => {
        if (valid) {
          try {
            ...
            const response = await popPrintApi.print(data)
            if (response.Success) {
    ...
    const handleExportExcel = async () => {
      ...
      try {
        ...
        const response = await popPrintApi.exportExcel(data)
        ...
        link.href = url
        link.download = `POP列印資料_${new Date().toISOString().slice(0, 10)}.xlsx`
        link.click()
    ...
    const handleSettings = async () => {
      try {
        const response = await popPrintApi.getSettings(null)
    ...
    const handleSettingsSave = async () => {
      try {
        const response = await popPrintApi.updateSettings(settingsForm.ShopId || null, settingsForm)
        if (response.Success) {
