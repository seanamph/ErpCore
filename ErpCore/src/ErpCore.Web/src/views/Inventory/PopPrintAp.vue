    ...
    >
      <el-form :model="settingsForm" ref="settingsFormRef" label-width="150px">
        ...
        <el-form-item label="報表類型">
          <el-input v-model="settingsForm.TypeId" placeholder="請輸入報表類型 (如: PR1_AP)" />
        </el-form-item>
...
import { popPrintApApi } from '@/api/popPrintAp'
  ...
  setup() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const response = await popPrintApApi.getProducts(params)
        if (response.Success) {
      ...
      await printFormRef.value.validate(async (valid) => {
        if (valid) {
          try {
            ...
            const response = await popPrintApApi.print(data)
            if (response.Success) {
    ...
    const handleExportExcel = async () => {
      ...
      try {
        ...
        const response = await popPrintApApi.exportExcel(data)
        ...
        link.href = url
        link.download = `POP列印資料_AP_${new Date().toISOString().slice(0, 10)}.xlsx`
        link.click()
    ...
    const handleSettings = async () => {
      try {
        const response = await popPrintApApi.getSettings(null)
    ...
    const handleSettingsSave = async () => {
      try {
        const response = await popPrintApApi.updateSettings(settingsForm.ShopId || null, settingsForm)
        if (response.Success) {
