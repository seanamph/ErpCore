...
├── Program.cs                      # 應用程式進入點（已配置所有 Repository 和 Service 依賴注入） ✅
├── Startup.cs                      # 啟動設定（.NET 7 使用 Program.cs，此檔案可選）✅（可選，已使用 Program.cs）
...
├── nlog.config                     # NLog 日誌設定（按小時分割，檔名格式：yyyyMMdd-HH.log） ✅
│
...
│   │   └── ProductCategoriesController.cs # ✅ 商品分類 (SYSB110) ✅
│   │
...
│   │   ├── PurchaseReceiptController.cs # ✅ 採購單驗收作業 (SYSW324, SYSW336, SYSW333, SYSW337, SYSW530) ✅
│   │   # ✅ 已日結調整相關功能已整合在 PurchaseReceiptController 中
│   │   # ✅ 資料庫SQL腳本 (CreatePurchaseReceiptTables.sql - 包含PurchaseReceipts, PurchaseReceiptDetails)
│   │   # ✅ Entity類別 (PurchaseReceipt, PurchaseReceiptDetail)
│   │   # ✅ Repository接口和實作 (IPurchaseReceiptRepository, PurchaseReceiptRepository)
│   │   # ✅ Service接口和實作 (IPurchaseReceiptService, PurchaseReceiptService)
│   │   # ✅ DTO類別 (PurchaseReceiptDto.cs - 包含驗收單、明細、查詢、建立、修改相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │   # ✅ Vue 前端頁面 (Purchase/PurchaseReceipt.vue) 已完成
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateTransferReceiptTables.sql - 包含TransferReceipts, TransferReceiptDetails)
│   │   # ✅ 資料庫SQL腳本 (CreateTransferReturnTables.sql - 包含TransferReturns, TransferReturnDetails)
│   │   # ✅ 資料庫SQL腳本 (CreateTransferShortageTables.sql - 包含TransferShortages, TransferShortageDetails)
│   │   # ✅ Entity類別 (TransferReceipt, TransferReceiptDetail, TransferReturn, TransferReturnDetail, TransferShortage, TransferShortageDetail)
│   │   # ✅ Repository接口和實作 (ITransferReceiptRepository, TransferReceiptRepository, ITransferReturnRepository, TransferReturnRepository, ITransferShortageRepository, TransferShortageRepository)
│   │   # ✅ Service接口和實作 (ITransferReceiptService, TransferReceiptService, ITransferReturnService, TransferReturnService, ITransferShortageService, TransferShortageService)
│   │   # ✅ DTO類別 (TransferReceiptDto.cs, TransferReturnDto.cs, TransferShortageDto.cs - 包含驗收單、驗退單、短溢單、明細、查詢、建立、修改相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
│   ├── InventoryCheck/             # 六、盤點管理類 (SYSW000) ✅
│   │   └── InventoryCheckController.cs # 盤點維護作業 (SYSW53M) ✅
│   │   # ✅ 資料庫SQL腳本 (CreateStocktakingPlanTables.sql - 包含StocktakingPlans, StocktakingPlanShops, StocktakingTemp等)
│   │   # ✅ Entity類別 (StocktakingPlan, StocktakingPlanShop, StocktakingTemp等)
│   │   # ✅ Repository接口和實作 (IStocktakingPlanRepository, StocktakingPlanRepository)
│   │   # ✅ Service接口和實作 (IStocktakingPlanService, StocktakingPlanService)
│   │   # ✅ DTO類別 (StocktakingPlanDto.cs - 包含盤點計劃、查詢、建立、修改相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │   # ✅ Vue 前端頁面 (InventoryCheck/StocktakingPlan.vue) 已完成
│   │
│   ├── StockAdjustment/           # 七、庫存調整類 (SYSW000) ✅
│   │   └── StockAdjustmentController.cs # 庫存調整作業 (SYSW490) ✅
│   │   # ✅ 資料庫SQL腳本 (CreateInventoryAdjustmentTables.sql - 包含InventoryAdjustments, InventoryAdjustmentDetails, AdjustmentReasons)
│   │   # ✅ Entity類別 (InventoryAdjustment, InventoryAdjustmentDetail, AdjustmentReason)
│   │   # ✅ Repository接口和實作 (IStockAdjustmentRepository, StockAdjustmentRepository)
│   │   # ✅ Service接口和實作 (IStockAdjustmentService, StockAdjustmentService)
│   │   # ✅ DTO類別 (InventoryAdjustmentDto.cs - 包含調整單、明細、查詢、建立、修改相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │   # ✅ Vue 前端頁面 (StockAdjustment/InventoryAdjustment.vue) 已完成
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateTaxAccountingTables.sql - 包含AccountSubjects, VoucherTypes, CommonVouchers, CashFlowLargeTypes, CashFlowMediumTypes, CashFlowSubjectTypes, CashFlowSubTotals, InvoiceVouchers, TaxReportPrints, TmpVoucherMs, TmpVoucherDs, VoucherImportLogs, VoucherImportDetails等)
│   │   # ✅ Entity類別 (TaxAccountingSubject, VoucherType, CommonVoucher, CashFlowLargeType, CashFlowMediumType, CashFlowSubjectType, CashFlowSubTotal, InvoiceVoucher, TaxReportPrint, TmpVoucherM, TmpVoucherD, VoucherImportLog, VoucherImportDetail等)
│   │   # ✅ Repository接口和實作 (ITaxAccountingSubjectRepository, TaxAccountingSubjectRepository, IVoucherTypeRepository, VoucherTypeRepository, ICommonVoucherRepository, CommonVoucherRepository, ICashFlowLargeTypeRepository, CashFlowLargeTypeRepository, ICashFlowMediumTypeRepository, CashFlowMediumTypeRepository, ICashFlowSubjectTypeRepository, CashFlowSubjectTypeRepository, ICashFlowSubTotalRepository, CashFlowSubTotalRepository, IInvoiceDataRepository, InvoiceDataRepository, ITransactionDataRepository, TransactionDataRepository, ITaxReportRepository, TaxReportRepository, ITaxReportPrintRepository, TaxReportPrintRepository, IVoucherAuditRepository, VoucherAuditRepository, IVoucherImportRepository, VoucherImportRepository)
│   │   # ✅ Service接口和實作 (ITaxAccountingSubjectService, TaxAccountingSubjectService, IVoucherTypeService, VoucherTypeService, ICommonVoucherService, CommonVoucherService, ICashFlowLargeTypeService, CashFlowLargeTypeService, ICashFlowMediumTypeService, CashFlowMediumTypeService, ICashFlowSubjectTypeService, CashFlowSubjectTypeService, ICashFlowSubTotalService, CashFlowSubTotalService, IInvoiceDataService, InvoiceDataService, ITransactionDataService, TransactionDataService, ITaxReportService, TaxReportService, ITaxReportPrintService, TaxReportPrintService, IVoucherAuditService, VoucherAuditService, IVoucherImportService, VoucherImportService)
│   │   # ✅ Controller (AccountingSubjectController, AccountingVoucherController, AccountingBookController, InvoiceDataController, TransactionDataController, TaxReportController, TaxReportPrintController, VoucherAuditController, VoucherImportController)
│   │   # ✅ DTO類別 (TaxAccountingDto.cs - 包含會計科目、憑證、帳簿、發票、交易、報表、傳票審核、傳票轉入相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateProcurementTables.sql - 包含Suppliers, Payments, BankAccounts, ProcurementReports等)
│   │   # ✅ Entity類別 (Supplier, Payment, BankAccount, ProcurementReport等)
│   │   # ✅ Repository接口和實作 (ISupplierRepository, SupplierRepository, IPaymentRepository, PaymentRepository, IBankAccountRepository, BankAccountRepository, IProcurementReportRepository, ProcurementReportRepository, IProcurementOtherRepository, ProcurementOtherRepository)
│   │   # ✅ Service接口和實作 (ISupplierService, SupplierService, IPaymentService, PaymentService, IBankAccountService, BankAccountService, IProcurementReportService, ProcurementReportService, IProcurementOtherService, ProcurementOtherService)
│   │   # ✅ Controller (ProcurementController, SupplierController, PaymentController, BankManagementController, ProcurementReportController, ProcurementOtherController)
│   │   # ✅ DTO類別 (ProcurementDto.cs - 包含採購、供應商、付款、銀行、報表相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateContractTables.sql - 包含Contracts, ContractTerms, ContractProcesses, ContractExtensions, CmsContracts等)
│   │   # ✅ Entity類別 (Contract, ContractTerm, ContractProcess, ContractExtension, CmsContract等)
│   │   # ✅ Repository接口和實作 (IContractRepository, ContractRepository, IContractProcessRepository, ContractProcessRepository等)
│   │   # ✅ Service接口和實作 (IContractService, ContractService, IContractProcessService, ContractProcessService等)
│   │   # ✅ Controller (ContractDataController, ContractProcessController, ContractExtensionController, CmsContractController)
│   │   # ✅ DTO類別 (ContractDto.cs - 包含合同、處理、擴展、CMS合同相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateLeaseTables.sql - 包含Leases, LeaseExtensions, LeaseProcesses等)
│   │   # ✅ Entity類別 (Lease, LeaseExtension, LeaseProcess等)
│   │   # ✅ Repository接口和實作 (ILeaseRepository, LeaseRepository, ILeaseExtensionRepository, LeaseExtensionRepository, ILeaseProcessRepository, LeaseProcessRepository)
│   │   # ✅ Service接口和實作 (ILeaseService, LeaseService, ILeaseExtensionService, LeaseExtensionService, ILeaseProcessService, LeaseProcessService)
│   │   # ✅ Controller (LeaseDataController, LeaseExtensionController, LeaseProcessController)
│   │   # ✅ DTO類別 (LeaseDto.cs - 包含租賃、擴展、處理相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
...
│   │   └── ReportModuleWPController.cs # 報表模組WP (SYSWP00) ✅ (位於 ReportExtension 目錄)
...
│   │   # ✅ Repository接口和實作 (IPcCashRepository, PcCashRepository, IPcCashRequestRepository, PcCashRequestRepository, IPcCashTransferRepository, PcCashTransferRepository, IPcCashInventoryRepository, PcCashInventoryRepository, IVoucherAuditRepository, VoucherAuditRepository) ✅
│   │   # ✅ Service接口和實作 (IPcCashService, PcCashService, IPcCashRequestService, PcCashRequestService, IPcCashTransferService, PcCashTransferService, IPcCashInventoryService, PcCashInventoryService, IVoucherAuditService, VoucherAuditService) ✅
│   │   # ✅ DTO類別 (QueryDto.cs - 包含所有處理和報表相關的DTO，包含 PcCashRequestDto, PcCashTransferDto, PcCashInventoryDto, UpdatePcCashRequestDto, UpdatePcCashTransferDto, PcCashTransferQueryDto, VoucherAuditDto, VoucherAuditQueryDto, UpdateVoucherAuditDto) ✅
│   │   # ✅ Controller API 端點 (QualityProcessController 包含 VoucherAudit 相關 API) ✅
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateCustomerInvoiceTables.sql - 包含CustomerData, CustomerContacts, CustomerAddresses, CustomerBankAccounts, Invoices, InvoiceDetails, InvoicePrintLogs, EmailFaxJobs, EmailFaxTemplates, EmailFaxLogs, GeneralLedger, Accounts, Vouchers, VoucherDetails, AccountBalances)
│   │   # ✅ Entity類別 (CustomerData, CustomerContact, CustomerAddress, CustomerBankAccount, Invoice, InvoiceDetail, InvoicePrintLog, EmailFaxJob, EmailFaxTemplate, EmailFaxLog, GeneralLedger, Account, Voucher, VoucherDetail, AccountBalance)
│   │   # ✅ Repository接口和實作 (ICustomerDataRepository, CustomerDataRepository, IInvoicePrintRepository, InvoicePrintRepository, IMailFaxRepository, MailFaxRepository, ILedgerDataRepository, LedgerDataRepository) ✅
│   │   # ✅ Service接口和實作 (ICustomerDataService, CustomerDataService, IInvoicePrintService, InvoicePrintService, IMailFaxService, MailFaxService, ILedgerDataService, LedgerDataService) ✅
│   │   # ✅ Controller (CustomerDataController, InvoicePrintController, MailFaxController, LedgerDataController) ✅
│   │   # ✅ DTO類別 (CustomerInvoiceDto.cs - 包含客戶資料、發票、郵件傳真、總帳相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateStoreMemberTables.sql - 包含Shops, ShopTypes, ShopPosSettings, Members, MemberLevels, MemberCards, MemberPoints, Promotions, StoreReports)
│   │   # ✅ Entity類別 (Shop, ShopType, ShopPosSetting, Member, MemberLevel, MemberCard, MemberPoint, Promotion, StoreReport)
│   │   # ✅ Repository接口和實作 (IStoreRepository, StoreRepository, IMemberRepository, MemberRepository, IPromotionRepository, PromotionRepository) ✅
│   │   # ✅ Service接口和實作 (IStoreService, StoreService, IMemberService, MemberService, IStoreQueryService, StoreQueryService, IMemberQueryService, MemberQueryService, IPromotionService, PromotionService, IStoreReportService, StoreReportService) ✅
│   │   # ✅ Controller (StoreController, StoreQueryController, MemberController, MemberQueryController, PromotionController, StoreReportController) ✅
│   │   # ✅ DTO類別 (StoreMemberDto.cs - 包含商店、會員、促銷活動、報表相關的DTO) ✅
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateStoreFloorTables.sql - 包含ShopFloors, Floors, ShopTypes, TypeCodes, ShopPosSettings, PosTerminals)
│   │   # ✅ Entity類別 (ShopFloor, Floor, TypeCode, PosTerminal)
│   │   # ✅ Repository接口和實作 (IShopFloorRepository, ShopFloorRepository, IFloorRepository, FloorRepository, ITypeCodeRepository, TypeCodeRepository, IPosTerminalRepository, PosTerminalRepository)
│   │   # ✅ Service接口和實作 (IShopFloorService, ShopFloorService, IShopFloorQueryService, ShopFloorQueryService, IFloorService, FloorService, IFloorQueryService, FloorQueryService, ITypeCodeService, TypeCodeService, ITypeCodeQueryService, TypeCodeQueryService, IPosTerminalService, PosTerminalService, IPosTerminalQueryService, PosTerminalQueryService)
│   │   # ✅ Controller (StoreManagementController, StoreQueryController, FloorController, FloorQueryController, TypeCodeController, TypeCodeQueryController, PosDataController, PosQueryController)
│   │   # ✅ DTO類別 (StoreFloorDto.cs, StoreQueryDto.cs, FloorDto.cs, FloorQueryDto.cs, TypeCodeDto.cs, TypeCodeQueryDto.cs, PosTerminalDto.cs, PosTerminalQueryDto.cs - 包含所有相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateInvoiceSalesTables.sql - 包含Invoices, InvoiceFormats, TaxRegistrations)
│   │   # ✅ 資料庫SQL腳本 (CreateElectronicInvoicePrintTables.sql - 包含ElectronicInvoices, ElectronicInvoicePrintSettings)
│   │   # ✅ 資料庫SQL腳本 (CreateReportPrintTables.sql - 包含ReportTemplates, ReportPrintLogs)
│   │   # ✅ Entity類別 (Invoice, InvoiceFormat, TaxRegistration, ElectronicInvoice, ElectronicInvoicePrintSetting, SalesOrder, SalesOrderDetail, ReportTemplate, ReportPrintLog)
│   │   # ✅ Repository接口和實作 (IInvoiceRepository, InvoiceRepository, IElectronicInvoiceRepository, ElectronicInvoiceRepository, IElectronicInvoicePrintSettingRepository, ElectronicInvoicePrintSettingRepository, ISalesOrderRepository, SalesOrderRepository, ISalesOrderDetailRepository, SalesOrderDetailRepository, ISalesOrderQueryRepository, SalesOrderQueryRepository, ISalesReportQueryRepository, SalesReportQueryRepository, IReportTemplateRepository, ReportTemplateRepository, IReportPrintLogRepository, ReportPrintLogRepository)
│   │   # ✅ Service接口和實作 (IInvoiceService, InvoiceService, IElectronicInvoiceService, ElectronicInvoiceService, IElectronicInvoicePrintSettingService, ElectronicInvoicePrintSettingService, ISalesOrderService, SalesOrderService, ISalesOrderQueryService, SalesOrderQueryService, ISalesReportQueryService, SalesReportQueryService, IReportPrintService, ReportPrintService)
│   │   # ✅ Controller (InvoiceDataController, InvoicePrintController, SalesDataController, SalesQueryController, SalesReportQueryController, SalesReportPrintController)
│   │   # ✅ DTO類別 (InvoiceDto.cs, ElectronicInvoiceDto.cs, SalesOrderDto.cs, SalesReportDto.cs, ReportPrintDto.cs - 包含發票、電子發票、銷售單、報表查詢、報表列印相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅ ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateB2BInvoiceSalesTables.sql - 包含B2BInvoices, B2BInvoiceFormats, B2BInvoiceTransfers, B2BElectronicInvoices, B2BElectronicInvoicePrintSettings, B2BSalesOrders, B2BSalesOrderDetails)
│   │   # ✅ Entity類別 (B2BInvoice, B2BElectronicInvoice, B2BElectronicInvoicePrintSetting, B2BSalesOrder, B2BSalesOrderDetail, B2BInvoiceTransfer)
│   │   # ✅ Repository接口和實作 (IB2BInvoiceRepository, B2BInvoiceRepository, IB2BElectronicInvoiceRepository, B2BElectronicInvoiceRepository, IB2BElectronicInvoicePrintSettingRepository, B2BElectronicInvoicePrintSettingRepository, IB2BSalesOrderRepository, B2BSalesOrderRepository, IB2BSalesOrderDetailRepository, B2BSalesOrderDetailRepository, IB2BSalesOrderQueryRepository, B2BSalesOrderQueryRepository)
│   │   # ✅ Service接口和實作 (IB2BInvoiceService, B2BInvoiceService, IB2BElectronicInvoiceService, B2BElectronicInvoiceService, IB2BElectronicInvoicePrintSettingService, B2BElectronicInvoicePrintSettingService, IB2BSalesOrderService, B2BSalesOrderService, IB2BSalesOrderQueryService, B2BSalesOrderQueryService)
│   │   # ✅ Controller (B2BInvoiceDataController, B2BInvoicePrintController, B2BSalesDataController, B2BSalesQueryController)
│   │   # ✅ DTO類別 (B2BInvoiceDto.cs, B2BElectronicInvoiceDto.cs, B2BSalesOrderDto.cs, B2BSalesOrderQueryDto.cs - 包含B2B發票、電子發票、銷售單、查詢、建立、修改相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateSystemExtensionETables.sql - 包含Employees, Personnel)
│   │   # ✅ Entity類別 (Employee, Personnel)
│   │   # ✅ Repository接口和實作 (IEmployeeRepository, EmployeeRepository, IPersonnelRepository, PersonnelRepository)
│   │   # ✅ Service接口和實作 (IEmployeeDataService, EmployeeDataService, IPersonnelDataService, PersonnelDataService)
│   │   # ✅ Controller (EmployeeDataController, PersonnelDataController)
│   │   # ✅ DTO類別 (SystemExtensionEDto.cs - 包含員工、人事、查詢、建立、修改相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateSystemExtensionHTables.sql - 包含PersonnelImportLogs, PersonnelImportDetails, EmpCard)
│   │   # ✅ Entity類別 (PersonnelImportLog, PersonnelImportDetail, EmpCard)
│   │   # ✅ Repository接口和實作 (IPersonnelImportLogRepository, PersonnelImportLogRepository, IPersonnelImportDetailRepository, PersonnelImportDetailRepository, IEmpCardRepository, EmpCardRepository)
│   │   # ✅ Service接口和實作 (IPersonnelBatchService, PersonnelBatchService, ISystemExtensionPHService, SystemExtensionPHService)
│   │   # ✅ DTO類別 (SystemExtensionHDto.cs - 包含人事匯入、感應卡相關的DTO)
│   │   # ✅ Controller (PersonnelBatchController, SystemExtensionPHController)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
│   ├── Loyalty/                     # 四十三、忠誠度系統類 (SYSLPS) ✅
│   │   ├── LoyaltyInitController.cs # ✅ 忠誠度系統初始化 (WEBLOYALTYINI) ✅
│   │   └── LoyaltyMaintenanceController.cs # 忠誠度系統維護 (LPS) ✅
│   │   # ✅ 資料庫SQL腳本 (CreateLoyaltyTables.sql - 包含LoyaltySystemConfigs, LoyaltySystemInitLogs, LoyaltyPointTransactions, LoyaltyMembers)
│   │   # ✅ Entity類別 (LoyaltySystemConfig, LoyaltySystemInitLog, LoyaltyPointTransaction, LoyaltyMember)
│   │   # ✅ Repository接口和實作 (ILoyaltySystemConfigRepository, LoyaltySystemConfigRepository, ILoyaltySystemInitLogRepository, LoyaltySystemInitLogRepository, ILoyaltyPointTransactionRepository, LoyaltyPointTransactionRepository, ILoyaltyMemberRepository, LoyaltyMemberRepository)
│   │   # ✅ Service接口和實作 (ILoyaltyInitService, LoyaltyInitService, ILoyaltyMaintenanceService, LoyaltyMaintenanceService)
│   │   # ✅ DTO類別 (LoyaltyDto.cs - 包含系統設定、初始化、點數交易、會員相關的DTO)
│   │   # ✅ Controller (LoyaltyInitController, LoyaltyMaintenanceController)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
│   ├── CustomerCustom/              # 四十四、客戶定制模組類 ✅
│   │   ├── Cus3000Controller.cs     # ✅ CUS3000系列 ✅ (會員管理、促銷活動管理、活動管理已完成)
...
│   │   # ✅ 資料庫SQL腳本 (CreateCustomerCustomTables.sql - 包含Cus3000Members, Cus3000Promotions, Cus3000Activities)
│   │   # ✅ Entity類別 (Cus3000Member, Cus3000Promotion, Cus3000Activity)
│   │   # ✅ Repository接口和實作 (ICus3000MemberRepository, Cus3000MemberRepository, ICus3000PromotionRepository, Cus3000PromotionRepository, ICus3000ActivityRepository, Cus3000ActivityRepository)
│   │   # ✅ Service接口和實作 (ICus3000MemberService, Cus3000MemberService, ICus3000PromotionService, Cus3000PromotionService, ICus3000ActivityService, Cus3000ActivityService)
│   │   # ✅ Controller (Cus3000Controller - 會員管理、促銷活動管理、活動管理已完成)
│   │   # ✅ DTO類別 (Cus3000Dto.cs - 包含會員、促銷活動、活動相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊
│   │
│   ├── StandardModule/             # 四十五、標準模組類 ✅
│   │   ├── Std3000Controller.cs     # ✅ STD3000系列 (SYS3620) ✅
│   │   └── Std5000Controller.cs     # STD5000系列 ✅
│   │   # ✅ 資料庫SQL腳本 (CreateStandardModuleTables.sql - 包含Std3000Data, Std5000BaseData, Std5000Members, Std5000MemberPoints, Std5000Transactions, Std5000TransactionDetails)
│   │   # ✅ Entity類別 (Std3000Data, Std5000BaseData, Std5000Member, Std5000MemberPoint, Std5000Transaction, Std5000TransactionDetail)
│   │   # ✅ Repository接口和實作 (IStd3000DataRepository, Std3000DataRepository, IStd5000BaseDataRepository, Std5000BaseDataRepository, IStd5000MemberRepository, Std5000MemberRepository, IStd5000MemberPointRepository, Std5000MemberPointRepository, IStd5000TransactionRepository, Std5000TransactionRepository, IStd5000TransactionDetailRepository, Std5000TransactionDetailRepository)
│   │   # ✅ Service接口和實作 (IStd3000DataService, Std3000DataService, IStd5000BaseDataService, Std5000BaseDataService, IStd5000MemberService, Std5000MemberService, IStd5000TransactionService, Std5000TransactionService)
│   │   # ✅ DTO類別 (Std3000DataDto.cs, Std5000Dto.cs - 包含基礎資料、會員、積分、交易、明細相關的DTO)
│   │   # ✅ Controller (Std3000Controller, Std5000Controller)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateMirModuleTables.sql - 包含MirH000Personnel, MirH000Salaries, MirV000Data, MirW000Data)
│   │   # ✅ Entity類別 (MirH000Personnel, MirH000Salary, MirV000Data, MirW000Data)
│   │   # ✅ Repository接口和實作 (IMirH000PersonnelRepository, MirH000PersonnelRepository, IMirH000SalaryRepository, MirH000SalaryRepository, IMirV000DataRepository, MirV000DataRepository, IMirW000DataRepository, MirW000DataRepository)
│   │   # ✅ Service接口和實作 (IMirH000PersonnelService, MirH000PersonnelService, IMirH000SalaryService, MirH000SalaryService, IMirV000DataService, MirV000DataService, IMirW000DataService, MirW000DataService)
│   │   # ✅ Controller (MirH000Controller - 人事管理、薪資管理已完成, MirV000Controller, MirW000Controller)
│   │   # ✅ DTO類別 (MirH000Dto.cs - 人事管理、薪資管理相關DTO已完成, MirV000Dto.cs, MirW000Dto.cs)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊
│   │
│   ├── MshModule/                   # 四十七、MSH模組類 ✅
│   │   └── Msh3000Controller.cs     # MSH3000系列 ✅
│   │   # ✅ 資料庫SQL腳本 (CreateMshModuleTables.sql - 包含Msh3000Data)
│   │   # ✅ Entity類別 (Msh3000Data)
│   │   # ✅ Repository接口和實作 (IMsh3000DataRepository, Msh3000DataRepository)
│   │   # ✅ Service接口和實作 (IMsh3000DataService, Msh3000DataService)
│   │   # ✅ Controller (Msh3000Controller)
│   │   # ✅ DTO類別 (Msh3000Dto.cs)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊
│   │
│   ├── SapIntegration/              # 四十八、SAP整合模組類 ✅
│   │   └── TransSapController.cs    # TransSAP系列 ✅
│   │   # ✅ 資料庫SQL腳本 (CreateSapIntegrationTables.sql - 包含TransSap)
│   │   # ✅ Entity類別 (TransSap)
│   │   # ✅ Repository接口和實作 (ITransSapRepository, TransSapRepository)
│   │   # ✅ Service接口和實作 (ITransSapService, TransSapService)
│   │   # ✅ Controller (TransSapController)
│   │   # ✅ DTO類別 (TransSapDto.cs)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊
│   │
│   ├── UniversalModule/              # 四十九、通用模組類 ✅
│   │   └── Univ000Controller.cs     # UNIV000系列 ✅
│   │   # ✅ 資料庫SQL腳本 (CreateUniversalModuleTables.sql - 包含Univ000)
│   │   # ✅ Entity類別 (Univ000)
│   │   # ✅ Repository接口和實作 (IUniv000Repository, Univ000Repository)
│   │   # ✅ Service接口和實作 (IUniv000Service, Univ000Service)
│   │   # ✅ Controller (Univ000Controller)
│   │   # ✅ DTO類別 (Univ000Dto.cs)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊
│   │
│   ├── CustomerCustomJgjn/          # 五十、客戶定制JGJN模組類 ✅
│   │   └── SysCustJgjnController.cs # SYSCUST_JGJN系列 ✅
│   │   # ✅ 資料庫SQL腳本 (CreateCustomerCustomJgjnTables.sql - 包含JgjNData, JgjNCustomer, JgjNInvoice)
│   │   # ✅ Entity類別 (JgjNData, JgjNCustomer, JgjNInvoice)
│   │   # ✅ Repository接口和實作 (IJgjNDataRepository, JgjNDataRepository, IJgjNCustomerRepository, JgjNCustomerRepository, IJgjNInvoiceRepository, JgjNInvoiceRepository)
│   │   # ✅ Service接口和實作 (IJgjNDataService, JgjNDataService, IJgjNCustomerService, JgjNCustomerService, IJgjNInvoiceService, JgjNInvoiceService)
│   │   # ✅ Controller (SysCustJgjnController - 資料管理、客戶管理、發票列印已完成)
│   │   # ✅ DTO類別 (JgjNDataDto.cs, JgjNCustomerDto.cs, JgjNInvoiceDto.cs - 包含所有相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateCommunicationModuleTables.sql - 包含SystemCommunications, CommunicationLogs, XComSystemParams)
│   │   # ✅ 資料庫SQL腳本 (CreateErrorMessageTables.sql - 包含ErrorMessages, ErrorMessageTemplates)
│   │   # ✅ Entity類別 (SystemCommunication, CommunicationLog, XComSystemParam, ErrorMessage, ErrorMessageTemplate)
│   │   # ✅ Repository接口和實作 (ISystemCommunicationRepository, SystemCommunicationRepository, IXComSystemParamRepository, XComSystemParamRepository, IErrorMessageRepository, ErrorMessageRepository)
│   │   # ✅ Service接口和實作 (ISystemCommunicationService, SystemCommunicationService, IXComSystemParamService, XComSystemParamService, IErrorMessageService, ErrorMessageService)
│   │   # ✅ Controller (XcomController, XcomMsgController)
│   │   # ✅ DTO類別 (CommunicationModuleDto.cs - 包含系統通訊設定、XCOM系統參數、錯誤訊息相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
│   ├── ChartTools/                   # 五十三、圖表與工具類 ✅
│   │   ├── ChartController.cs       # ✅ 圖表功能 (SYS1000) ✅
│   │   └── ToolsController.cs       # ✅ 工具功能 ✅
│   │   # ✅ 資料庫SQL腳本 (CreateChartToolsTables.sql - 包含ChartConfigs)
│   │   # ✅ Entity類別 (ChartConfig)
│   │   # ✅ Repository接口和實作 (IChartConfigRepository, ChartConfigRepository)
│   │   # ✅ Service接口和實作 (IChartConfigService, ChartConfigService)
│   │   # ✅ Controller (ChartController)
│   │   # ✅ DTO類別 (IChartConfigService.cs - 包含圖表配置相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateInvoiceExtensionTables.sql - 包含EInvoiceExtensions)
│   │   # ✅ Entity類別 (EInvoiceExtension)
│   │   # ✅ Repository接口和實作 (IEInvoiceExtensionRepository, EInvoiceExtensionRepository)
│   │   # ✅ Service接口和實作 (IEInvoiceExtensionService, EInvoiceExtensionService)
│   │   # ✅ Controller (InvoiceExtensionController)
│   │   # ✅ DTO類別 (IEInvoiceExtensionService.cs - 包含電子發票擴展相關的DTO)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateSalesReportTables.sql - 包含SalesReports, ProductReports, CrystalReportSettings)
│   │   # ✅ Entity類別 (SalesReport, ProductReport, CrystalReportSetting)
│   │   # ✅ Repository接口和實作 (ISalesReportRepository, SalesReportRepository)
│   │   # ✅ Service接口和實作 (ISalesReportService, SalesReportService)
│   │   # ✅ DTO類別 (SalesReportDto.cs - 包含銷售報表、查詢、建立、修改、生成報表相關的DTO)
│   │   # ✅ Controller (SalesReportModuleController, CrystalReportController)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
...
│   │   # ✅ 資料庫SQL腳本 (CreateEnergyTables.sql - 包含EnergyBases, EnergyProcesses, EnergyExtensions)
│   │   # ✅ Entity類別 (EnergyBase, EnergyProcess, EnergyExtension)
│   │   # ✅ Repository接口和實作 (IEnergyBaseRepository, EnergyBaseRepository, IEnergyProcessRepository, EnergyProcessRepository, IEnergyExtensionRepository, EnergyExtensionRepository)
│   │   # ✅ Service接口和實作 (IEnergyBaseService, EnergyBaseService, IEnergyProcessService, EnergyProcessService, IEnergyExtensionService, EnergyExtensionService)
│   │   # ✅ DTO類別 (EnergyDto.cs - 包含能源基礎、處理、擴展資料相關的DTO)
│   │   # ✅ Controller (EnergyBaseController, EnergyProcessController, EnergyExtensionController)
│   │   # ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
│   │
│   └── Common/                       # 共用控制器
...
│   │   │   ├── IUserService.cs ✅
│   │   │   ├── IRoleService.cs ✅
│   │   │   ├── IPermissionService.cs ✅ (使用 ISystemPermissionService, IUserPermissionService 等)
│   │   │   ├── ISystemConfigService.cs ✅ (使用 SystemConfig 相關服務)
│   │   │   └── ILogService.cs ✅ (使用 IChangeLogService, IButtonLogService 等)
│   │   ├── BasicData/              # 基本資料服務介面
...
│       │   ├── UserService.cs ✅
│       │   ├── RoleService.cs ✅
│       │   ├── PermissionService.cs ✅ (使用 SystemPermissionService, UserPermissionService 等)
│       │   ├── SystemConfigService.cs ✅ (使用 SystemConfig 相關服務)
│       │   └── LogService.cs ✅ (使用 ChangeLogService, ButtonLogService 等)
│       ├── BasicData/              # 基本資料服務實作
│       │   ├── ParameterService.cs ✅
│       │   ├── RegionService.cs ✅
│       │   ├── AreaService.cs ✅
...
│   │   ├── ProspectDto.cs ✅
│   │   ├── InterviewDto.cs ✅
│   │   └── TenantLocationDto.cs ✅ (BusinessOther 使用 TenantLocationDto)
...
│   │   ├── Prospect.cs ✅              # 潛客
│   │   ├── Interview.cs ✅              # 訪談
│   │   └── TenantLocation.cs         # 租戶位置 (SYSC999) ✅
│   │
...
│       │   ├── ItemPermissionRepository.cs ✅
│       │   ├── RoleFieldPermissionRepository.cs ✅
│       │   ├── UserFieldPermissionRepository.cs ✅
│       │   ├── ItemCorrespondRepository.cs ✅
│       │   ├── ChangeLogRepository.cs ✅
│       │   ├── ButtonLogRepository.cs ✅
│       │   ├── ControllableFieldRepository.cs ✅
...
│       │   ├── SupplierGoodsRepository.cs ✅
│       │   ├── ProductGoodsIdRepository.cs ✅
│       │   ├── PriceChangeRepository.cs ✅
│       │   ├── PopPrintRepository.cs ✅
│       │   ├── TextFileProcessLogRepository.cs ✅
│       │   ├── TextFileProcessDetailRepository.cs ✅
│       │   ├── StockRepository.cs ✅
...
│       ├── InventoryCheck/          # 盤點儲存庫 (SYSW000)
│       │   └── StocktakingPlanRepository.cs ✅
│       ├── StockAdjustment/         # 庫存調整儲存庫 (SYSW000)
│       │   └── StockAdjustmentRepository.cs ✅
│       ├── Invoice/                # 電子發票儲存庫 (ECA0000)
│       │   ├── InvoiceRepository.cs ✅
│       │   └── InvoiceReportRepository.cs
│       ├── EInvoice/                # 電子發票儲存庫 (ECA0000)
│       │   └── EInvoiceRepository.cs ✅
...
│       │   ├── EmployeeMealCardRepository.cs ✅
│       │   ├── ReturnCardRepository.cs ✅
│       │   └── OvertimeRepository.cs
│       │   └── OvertimePaymentRepository.cs ✅
│       ├── Pos/                    # POS儲存庫
│       │   ├── PosTransactionRepository.cs ✅
│       │   └── PosReportRepository.cs
│       ├── SystemExtension/        # 系統擴展儲存庫
│       │   └── SystemExtensionRepository.cs ✅
│       ├── SystemConfig/            # 系統設定儲存庫
│       │   ├── ConfigSystemRepository.cs ✅
│       │   ├── ConfigSubSystemRepository.cs ✅
│       │   ├── ConfigProgramRepository.cs ✅
│       │   └── ConfigButtonRepository.cs ✅
│       ├── Kiosk/                  # 自助服務終端儲存庫
...
│       │   ├── EmailAttachmentRepository.cs ✅
│       │   └── EncodeLogRepository.cs ✅
│       ├── UiComponent/            # UI組件儲存庫 ✅
│       │   └── UIComponentRepository.cs ✅
│       ├── Tools/                   # 工具儲存庫 ✅
│       │   ├── FileUploadRepository.cs ✅
│       │   └── PdfConversionLogRepository.cs ✅ (Html2Pdf 使用此 Repository)
...
│       │   ├── LoyaltyMemberRepository.cs ✅
│       │   ├── LoyaltyPointTransactionRepository.cs ✅
│       │   ├── LoyaltySystemConfigRepository.cs ✅
│       │   └── LoyaltySystemInitLogRepository.cs ✅
...
│       │   └── TenantLocationRepository.cs ✅
│       ├── Recruitment/            # 招商儲存庫 (SYSC000) - 別名
│       │   ├── TenantLocationRepository.cs ✅
│       │   └── BusinessOtherRepository.cs ✅
│       ├── CommunicationModule/     # 通訊模組儲存庫 (XCOM000)
│       │   ├── XcomRepository.cs
│       │   ├── XcomMsgRepository.cs # XCOMMSG錯誤訊息處理儲存庫
│       │   ├── ErrorMessageRepository.cs ✅
│       │   ├── XComSystemParamRepository.cs ✅
│       │   └── SystemCommunicationRepository.cs ✅
│       ├── ChartTools/             # 圖表與工具儲存庫
│       │   ├── ChartRepository.cs
│       │   ├── ChartConfigRepository.cs ✅
│       │   └── ToolsRepository.cs
│       ├── SystemExit/             # 系統退出儲存庫
│       │   └── SystemExitRepository.cs
│       ├── InvoiceExtension/       # 電子發票擴展儲存庫
│       │   └── EInvoiceExtensionRepository.cs ✅
│       ├── SalesReport/            # 銷售報表儲存庫 (SYS1000)
│       │   └── SalesReportRepository.cs ✅
│       ├── Energy/                 # 能源儲存庫 (SYSO000)
│       │   ├── EnergyBaseRepository.cs ✅
│       │   ├── EnergyProcessRepository.cs ✅
│       │   └── EnergyExtensionRepository.cs ✅
│       └── Common/                 # 共用儲存庫
...
│   │   │   ├── Users.vue           # 使用者基本資料維護 (SYS0110) ✅
│   │   │   ├── Roles.vue           # 角色基本資料維護 (SYS0210) ✅
│   │   │   ├── Permissions.vue     # 權限管理 (SYS0310, SYS0320, SYS0330, SYS0340) ✅
│   │   │   ├── SystemConfig.vue    # 系統設定 (CFG0410, CFG0420, CFG0430, CFG0440) ✅
│   │   │   ├── ChangeLogs.vue      # 異動記錄查詢 (SYS0610-SYS0660) ✅
│   │   │   ├── UserQueryReport.vue # 使用者查詢功能 (SYS0910) ✅
│   │   │   ├── SystemProgramButtonReport.vue # 系統作業與功能列表查詢 (SYS0810, SYS0999) ✅
│   │   │   ├── PermissionCategoryReport.vue # 權限分類報表查詢 (SYS0770) ✅
│   │   │   ├── RoleUserListReport.vue # 角色之使用者列表 (SYS0750) ✅
│   │   │   ├── RoleSystemPermissionListReport.vue # 角色系統權限列表 (SYS0731) ✅
│   │   │   ├── ProgramRolePermissionListReport.vue # 作業權限之角色列表 (SYS0740) ✅
│   │   │   ├── ProgramUserPermissionReport.vue # 作業權限之使用者列表 (SYS0720) ✅
│   │   │   ├── SystemPermissionReport.vue # 系統權限列表 (SYS0710) ✅
│   │   │   └── ButtonLogReport.vue # BUTTON LOG 查詢 (SYS0790) ✅
│   │   │
│   │   ├── BasicData/              # 二、基本資料管理 (SYSB000) ✅
│   │   │   ├── Parameters.vue     # 參數設定 (SYSBC40) ✅
│   │   │   ├── Regions.vue         # 地區設定 (SYSBC30) ✅
│   │   │   ├── Areas.vue          # 區域設定 (SYSB450) ✅
│   │   │   ├── Banks.vue          # 金融機構 (SYSBC20) ✅
│   │   │   ├── Vendors.vue        # 廠商客戶 (SYSB206) ✅
│   │   │   ├── Departments.vue    # 部別資料維護 (SYSWB40) ✅
│   │   │   ├── Groups.vue         # 組別資料維護 (SYSWB70) ✅
│   │   │   ├── Warehouses.vue    # 庫別資料維護 (SYSWB60) ✅
│   │   │   └── ProductCategory.vue # 商品分類資料維護 (SYSB110) ✅
│   │   │
│   │   ├── Inventory/              # 三、進銷存管理 (SYSW000) ✅
│   │   │   ├── SupplierGoods.vue  # 供應商商品資料維護 (SYSW110) ✅
│   │   │   ├── ProductGoodsId.vue # 商品進銷碼維護 (SYSW137) ✅
│   │   │   ├── PriceChange.vue   # 商品永久變價 (SYSW150) ✅
│   │   │   ├── PopPrint.vue      # POP卡商品卡列印 (SYSW170) ✅
│   │   │   ├── PopPrintAp.vue    # POP卡商品卡列印_AP (SYSW171) ✅
│   │   │   └── PopPrintUa.vue    # POP卡商品卡列印_UA (SYSW172) ✅
│   │   │
│   │   ├── Purchase/              # 四、採購管理 (SYSP000) ✅
│   │   │   ├── PurchaseOrder.vue  # 訂退貨申請作業 (SYSW315) ✅
│   │   │   ├── PurchaseOrderReturn.vue # 訂退貨申請作業 (SYSW316) ✅
│   │   │   ├── OnSitePurchaseOrder.vue # 現場打單作業 (SYSW322) ✅
│   │   │   ├── PurchaseReceipt.vue # 採購單驗收作業 (SYSW324, SYSW336) ✅
│   │   │   ├── SettledPurchaseReceiptAdjustment.vue # 已日結採購單驗收調整 (SYSW333) ✅
│   │   │   ├── ClosedReturnAdjustment.vue # 已日結退貨單驗退調整 (SYSW337) ✅
│   │   │   └── ClosedReturnAdjustmentV2.vue # 已日結退貨單驗退調整 (SYSW530) ✅
│   │   │
...
│   │   │   └── StocktakingPlan.vue # 盤點維護作業 (SYSW53M) ✅
│   │   │
│   │   ├── StockAdjustment/       # 七、庫存調整 (SYSW000) ✅
│   │   │   └── InventoryAdjustment.vue # 庫存調整作業 (SYSW490) ✅
│   │   │
│   │   ├── EInvoice/              # 八、電子發票管理 (ECA0000) ✅
│   │   │   ├── EInvoiceUpload.vue # 電子發票上傳 (ECA2050, ECA3010, ECA3030) ✅
│   │   │   ├── EInvoiceProcess.vue # 電子發票處理 (ECA3010) ✅
│   │   │   ├── EInvoiceQuery.vue # 電子發票查詢 (ECA3020) ✅
│   │   │   └── EInvoiceReport.vue # 電子發票報表 (ECA3040) ✅
│   │   │
│   │   ├── Customer/              # 九、客戶管理 (CUS5000) ✅
│   │   │   ├── CustomerMaintenance.vue # 客戶基本資料維護 (CUS5110) ✅
│   │   │   ├── CustomerQuery.vue  # 客戶查詢作業 (CUS5120) ✅
│   │   │   └── CustomerReport.vue # 客戶報表 (CUS5130) ✅
│   │   │
│   │   ├── AnalysisReport/         # 十、分析報表 (SYSA000) ✅
│   │   │   └── AnalysisReportQuery.vue # 進銷存分析報表 (SYSA1000) ✅
│   │   │
│   │   ├── BusinessReport/        # 十一、業務報表 (SYSL000) ✅
│   │   │   └── EmployeeMealCardList.vue # 員餐卡管理 (SYSL210) ✅
│   │   │
│   │   ├── Pos/                    # 十二、POS系統 ✅
...
│   │   │   └── SYSX140Report.vue  # 系統擴展報表 (SYSX140) ✅
│   │   │
│   │   ├── Kiosk/                  # 十四、自助服務終端 ✅
...
│   │   │   ├── ModuleN.vue         # 報表模組N ✅
│   │   │   ├── ModuleWP.vue        # 報表模組WP ✅
│   │   │   ├── Module7.vue         # 報表模組7 (SYS7000) ✅
│   │   │   ├── Print.vue           # 報表列印作業 (SYS7B10-SYS7B40) ✅
│   │   │   └── Statistics.vue      # 報表統計作業 (SYS7C10, SYS7C30) ✅
│   │   │
│   │   ├── DropdownList/           # 十六、下拉列表 ✅
│   │   │   ├── AddressList.vue    # 地址列表 (ADDR_CITY_LIST, ADDR_ZONE_LIST) ✅
│   │   │   ├── DateList.vue       # 日期列表 (DATE_LIST) ✅
│   │   │   ├── MenuList.vue       # 選單列表 (MENU_LIST) ✅
│   │   │   ├── MultiSelectList.vue # 多選列表 (MULTI_AREA_LIST, MULTI_SHOP_LIST, MULTI_USERS_LIST) ✅
│   │   │   └── SystemList.vue     # 系統列表 (SYSID_LIST, USER_LIST) ✅
│   │   │
...
│   │   │
│   │   ├── UiComponent/            # 十八、UI組件 ✅
│   │   │   ├── DataMaintenance.vue # 資料維護UI組件 (IMS30系列) ✅
│   │   │   └── QueryReport.vue    # UI組件查詢與報表 ✅
│   │   │
│   │   ├── Tools/                   # 十九、工具 ✅
...
│   │   │
│   │   ├── Core/                    # 二十、核心功能
│   │   │   ├── UserManagement.vue  # 使用者管理 ✅
│   │   │   ├── Framework.vue        # 框架功能 ✅
│   │   │   ├── DataMaintenance.vue  # 資料維護功能 ✅ (位於UiComponent目錄)
│   │   │   ├── Tools.vue            # 工具功能 ✅
│   │   │   └── SystemFunction.vue   # 系統功能 ✅
│   │   │
│   │   ├── OtherModule/            # 二十一、其他模組
│   │   │   ├── CrpReport.vue       # CRP報表模組 ✅
│   │   │   ├── EipIntegration.vue  # EIP系統整合 (IMS2EIP) ✅
│   │   │   └── LabTest.vue         # 實驗室測試功能 (Lab) ✅
│   │   │
...
│   │   │   ├── AccountSubject.vue  # 會計科目維護 (SYSN110) ✅
│   │   │   ├── Accounting.vue      # 會計管理 (SYSN120-SYSN154) ✅
│   │   │   ├── FinancialTransaction.vue # 財務交易 (SYSN210-SYSN213) ✅
│   │   │   ├── Asset.vue          # 資產管理 (SYSN310-SYSN311) ✅
│   │   │   ├── FinancialReport.vue # 財務報表 (SYSN510-SYSN540) ✅
│   │   │   └── OtherFinancial.vue # 其他財務功能 (SYSN610-SYSN910) ✅
│   │   │
│   │   ├── TaxAccounting/          # 二十四、會計稅務管理 (SYST000) ✅
│   │   │   ├── AccountingSubject.vue  # 會計科目維護 (SYST111-SYST11A) ✅
│   │   │   ├── AccountingVoucher.vue  # 會計憑證管理 (SYST121-SYST12B) ✅
│   │   │   ├── AccountingBook.vue     # 會計帳簿管理 (SYST131-SYST134) ✅
│   │   │   ├── InvoiceData.vue        # 發票資料維護 (SYST211-SYST212) ✅
│   │   │   ├── TransactionData.vue    # 交易資料處理 (SYST221, SYST311-SYST352) ✅
│   │   │   ├── TaxReport.vue          # 稅務報表查詢 (SYST411-SYST452) ✅
│   │   │   ├── TaxReportPrint.vue     # 稅務報表列印 (SYST510-SYST530) ✅
│   │   │   ├── VoucherAudit.vue       # 暫存傳票審核作業 (SYSTA00-SYSTA70) ✅
│   │   │   └── VoucherImport.vue      # 傳票轉入作業 (SYST002-SYST003) ✅
│   │   │
│   │   ├── Procurement/            # 二十五、採購供應商管理 (SYSP000)
│   │   │   ├── Procurement.vue    # 採購管理 (SYSP110-SYSP190) ✅
│   │   │   ├── Supplier.vue        # 供應商管理 (SYSP210-SYSP260) ✅
│   │   │   ├── Payment/            # 付款管理 (SYSP271-SYSP2B0) ✅
│   │   │   │   └── index.vue       ✅
│   │   │   ├── BankManagement/     # 銀行管理 ✅
│   │   │   │   └── index.vue       ✅
│   │   │   ├── ProcurementReport/  # 採購報表 (SYSP410-SYSP4I0) ✅
│   │   │   │   └── index.vue       ✅
│   │   │   └── ProcurementOther/   # 採購其他功能 (SYSP510-SYSP530) ✅
│   │   │       └── index.vue       ✅
│   │   │
...
│   │   │   │   └── index.vue        ✅ (實際位於 Recruitment/ProspectMasterList.vue)
│   │   │   ├── Prospect/            # 潛客 (SYSC180) ✅
│   │   │   │   └── index.vue        ✅ (實際位於 Recruitment/ProspectList.vue)
│   │   │   ├── Interview/           # 訪談 (SYSC222) ✅
│   │   │   │   └── index.vue         ✅ (實際位於 Recruitment/InterviewList.vue)
│   │   │   └── BusinessOther/       # 招商其他功能 (SYSC999) ✅
│   │   │       └── index.vue         ✅ (實際位於 Recruitment/TenantLocationList.vue)
│   │   │
...
24. ✅ **會計稅務管理類 (SYST000)** ✅
    ...
    - 前端：`ErpCore.Web/src/views/TaxAccounting/`、`ErpCore.Web/src/api/taxAccounting.js`、路由配置 ✅
    - 資料庫：`database/Scripts/Schema/Tables/TaxAccounting/`、`database/Migrations/TaxAccounting/`

25. ✅ **採購供應商管理類 (SYSP000)** - 已完成
    ...
    - ✅ PaymentController - 付款管理 (SYSP271-SYSP2B0) ✅
    - ✅ BankManagementController - 銀行管理 ✅
    - ✅ ProcurementReportController - 採購報表 (SYSP410-SYSP4I0) ✅
    - ✅ ProcurementOtherController - 採購其他功能 (SYSP510-SYSP530等) ✅
    - ✅ Repository接口和實作：ISupplierRepository, IPaymentRepository, IBankAccountRepository, IProcurementOtherRepository ✅
    - ✅ Service接口和實作：ISupplierService, IPaymentService, IBankAccountService, IProcurementReportService, IProcurementOtherService ✅
    - ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅

26. ✅ **合同管理類 (SYSF000)** - 已完成
    ...
    - ✅ ContractDataController - 合同資料維護 (SYSF110-SYSF140) ✅
    - ✅ ContractProcessController - 合同處理作業 (SYSF210-SYSF220) ✅
    - ✅ ContractExtensionController - 合同擴展維護 (SYSF350-SYSF540) ✅
    - ✅ CmsContractController - CMS合同維護 (CMS2310-CMS2320) ✅
    - ✅ Repository接口和實作：IContractRepository, IContractExtensionRepository, IContractProcessRepository, ICmsContractRepository ✅
    - ✅ Service接口和實作：IContractService, IContractExtensionService, IContractProcessService, ICmsContractService ✅
    - ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅

27. ✅ **租賃管理類 (SYS8000)** - 已完成
    - 後端：`ErpCore.Api/Controllers/Lease/`、`ErpCore.Application/Services/Lease/`、`ErpCore.Domain/Entities/Lease/`
    - 前端：`ErpCore.Web/src/views/Lease/`、`ErpCore.Web/src/api/lease.js`、`ErpCore.Web/src/router/index.js` ✅
    - 資料庫：`database/Scripts/CreateLeaseTables.sql` ✅
    - ✅ LeaseDataController - 租賃資料維護 (SYS8110-SYS8220) ✅
    - ✅ LeaseExtensionController - 租賃擴展維護 (SYS8A10-SYS8A45) ✅
    - ✅ LeaseProcessController - 租賃處理作業 (SYS8B50-SYS8B90) ✅
    - ✅ Repository接口和實作：ILeaseRepository, ILeaseExtensionRepository, ILeaseProcessRepository ✅
    - ✅ Service接口和實作：ILeaseService, ILeaseExtensionService, ILeaseProcessService ✅
    - ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
    - ✅ 前端頁面：LeaseData/index.vue、LeaseExtension/index.vue、LeaseProcess/index.vue ✅
    - ✅ API 檔案：lease.js ✅
    - ✅ 路由配置：已在 router/index.js 註冊所有頁面路由 ✅

28. ✅ **租賃管理SYSE類 (SYSE000)** - 已完成
    - 後端：`ErpCore.Api/Controllers/Lease/LeaseSYSEDataController.cs`、`ErpCore.Api/Controllers/Lease/LeaseSYSEFeeController.cs` ✅
    - 前端：`ErpCore.Web/src/views/LeaseSYSE/`、`ErpCore.Web/src/api/leaseSYSE.js`、`ErpCore.Web/src/router/index.js` ✅
    - 資料庫：`database/Scripts/CreateLeaseSYSETables.sql` ✅
    - ✅ LeaseSYSEDataController - 租賃資料維護 (SYSE110-SYSE140) ✅
    - ✅ LeaseSYSEFeeController - 費用資料維護 (SYSE310-SYSE430) ✅
    - ✅ Repository接口和實作：ILeaseTermRepository, ILeaseAccountingCategoryRepository, ILeaseFeeRepository, ILeaseFeeItemRepository ✅
    - ✅ Service接口和實作：ILeaseTermService, ILeaseAccountingCategoryService, ILeaseFeeService, ILeaseFeeItemService ✅
    - ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
    - ✅ 前端頁面：LeaseSYSEData/index.vue (SYSE110-SYSE140) ✅、LeaseSYSEExtension/index.vue (SYSE210-SYSE230) ✅
    - ✅ 前端頁面：LeaseSYSEFee/index.vue (SYSE310-SYSE430) ✅
    - ✅ API 檔案：leaseSYSE.js ✅
    - ✅ 路由配置：已在 router/index.js 註冊所有頁面路由 ✅

29. ✅ **租賃管理SYSM類 (SYSM000)** - 已完成
    - 後端：`ErpCore.Api/Controllers/Lease/LeaseSYSMDataController.cs`、`ErpCore.Api/Controllers/Lease/LeaseSYSMReportController.cs` ✅
    - 前端：`ErpCore.Web/src/views/LeaseSYSM/`、`ErpCore.Web/src/api/leaseSYSM.js`、`ErpCore.Web/src/router/index.js` ✅
    - 資料庫：`database/Scripts/CreateLeaseSYSMTables.sql` ✅
    - ✅ LeaseSYSMDataController - 租賃資料維護 (SYSM111-SYSM138) ✅
    - ✅ LeaseSYSMReportController - 租賃報表查詢 (SYSM141-SYSM144) ✅
    - ✅ Repository接口和實作：IParkingSpaceRepository, ILeaseContractRepository, ILeaseReportQueryRepository ✅
    - ✅ Service接口和實作：IParkingSpaceService, ILeaseContractService, ILeaseReportQueryService ✅
    - ✅ 依賴注入註冊：已在 Program.cs 註冊所有 Repository 和 Service ✅
    - ✅ 前端頁面：LeaseSYSMData/index.vue (SYSM111-SYSM138) ✅
    - ✅ 前端頁面：LeaseSYSMReport/index.vue (SYSM141-SYSM144) ✅
    - ✅ API 檔案：leaseSYSM.js ✅
    - ✅ 路由配置：已在 router/index.js 註冊所有頁面路由 ✅

30. ✅ **擴展管理類 (SYS9000)**
    - 後端：`ErpCore.Api/Controllers/Extension/`、`ErpCore.Application/Services/Extension/`、`ErpCore.Domain/Entities/Extension/`
    - ✅ 前端頁面：Extension/index.vue ✅
    - ✅ 前端 API：extension.js ✅
    - ✅ 前端路由：已在 router/index.js 中註冊 ✅
...
55. ✅ **電子發票擴展類 (ECA1000)** - 已完成
    ...
    - 資料庫：`database/Scripts/CreateInvoiceExtensionTables.sql`
    - Entity：EInvoiceExtension
    - Repository、Service、Controller、DTO：已完成基本結構
    - 依賴注入註冊：已在 Program.cs 註冊
